"""Discriminating checks of transient previews; never edits the source tree.
...
def main():
    ...
    preview = 'src/AccessibleMediaController.Windows/MainWindow.TransientPreviews.cs'
    model = 'src/AccessibleMediaController.Core/Presentation/TransientPreviewNavigation.cs'
    mutations = [
        ('green', None, None, None),
        ('radio-keeps-stale-return', main_window,
         'if (commandId == CommandIds.ViewRadio)\n        {\n            NoteExplicitNavigationAwayFromPreview();',
         'if (commandId == CommandIds.ViewRadio)\n        {'),
        ('inbox-replaces-playing-catalog', preview,
         '        NavigateTo(PodcastInboxViewName);',
         '        ReloadPodcastSessionItems();\n        NavigateTo(PodcastInboxViewName);'),
        ('lose-filter-focus', preview,
         'FilterFocused: FilterBox?.IsKeyboardFocusWithin == true', 'FilterFocused: false'),
        ('block-wiim-preview', main_window,
         '        IsTransientPreviewCommand(commandId)\n        || CommandIds.TryParseRadioPreset',
         '        false\n        || CommandIds.TryParseRadioPreset'),
        ('wrong-view-invalidation', main_window,
         'if (!previewDestination) NoteExplicitNavigationAwayFromPreview();',
         'if (previewDestination) NoteExplicitNavigationAwayFromPreview();'),
        ('lose-player-view', model,
         'var restorePlayer = origin.PlayerActive;', 'var restorePlayer = false;'),
...
dotnet run --project tests/AccessibleMediaController.Windows.SmokeTests -c Release --no-build --disable-build-servers -- --transient-preview-acceptance
exit $LASTEXITCODE
""".replace("'DOTNET'", "'" + args.dotnet_root + "'").replace("'NUGET'", "'" + args.nuget_source + "'").replace("'REPO'", "'" + windows(copy) + "'"), encoding='utf-8-sig')
            ...
            counts = re.search(r'PODGLADY: (\d+) OK / (\d+) BLAD / razem (\d+)', text)
            if not counts:
