"""Discriminating checks of playback persistence; never edits the source tree.
...
This is a focused headless test, not the full build.ps1 release gate.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    queue = 'src/AccessibleMediaController.Windows/Services/StatePersistenceQueue.cs'
    checkpoint = 'src/AccessibleMediaController.Windows/Services/PlaybackStateCheckpoint.cs'
    mutations = [
        ('green', None, None, None),
        ('full-clone-in-checkpoint', queue,
         '        lock (_gate)\n        {\n            if (_hasSnapshot)',
         '        _ = _clone(state);\n        lock (_gate)\n        {\n            if (_hasSnapshot)'),
        ('stale-checkpoint-after-full-save', queue,
         '            _hasSnapshot = true;\n            _pendingCheckpoints.Clear();',
         '            _hasSnapshot = true;'),
        ('drop-earlier-podcast', queue, '? checkpoint.MergeEarlier(older)', '? checkpoint'),
        ('drop-spotify-position', checkpoint,
         '                JsonSerializer.Deserialize<SpotifyPlaybackSettings>(_spotifyPlayback)!;',
         '                new SpotifyPlaybackSettings();'),
...
dotnet run --project tests/AccessibleMediaController.Windows.SmokeTests -c Release --no-build --disable-build-servers -- --periodic-playback-checkpoint
exit $LASTEXITCODE
""".replace("'DOTNET'", "'" + args.dotnet_root + "'").replace("'NUGET'", "'" + args.nuget_source + "'").replace("'REPO'", "'" + windows(copy) + "'"), encoding='utf-8-sig')
            ...
            counts = re.search(r'Checkpointy: (\d+) OK / (\d+) BLAD / razem (\d+)', text)
            if not counts:
