"""Discriminating checks of Spotify relations navigation; never edits the source tree.
...
def main():
    ...
    ui='--spotify-relations'
    mutations=[
        ('green-ui',None,None,None,ui),
        ('album-content-in-menu',main_window,'else if (CanOpenSpotifyContainer(item) && item.Kind != MediaItemKind.Album)','else if (CanOpenSpotifyContainer(item))',ui),
        ('albumy-opens-tracks',main_window,'actions.Add(("Albumy",\n                () => _ = OpenSpotifyContainerAsync(item, ArtistBrowseSection.Albums)))','actions.Add(("Albumy",\n                () => _ = OpenSpotifyContainerAsync(item, ArtistBrowseSection.Tracks)))',ui),
        ('utwory-opens-albums',main_window,'actions.Add(("Utwory",\n                () => _ = OpenSpotifyContainerAsync(item, ArtistBrowseSection.Tracks)))','actions.Add(("Utwory",\n                () => _ = OpenSpotifyContainerAsync(item, ArtistBrowseSection.Albums)))',ui),
...
""".replace("'DOTNET'", "'" + args.dotnet_root + "'").replace("'NUGET'", "'" + args.nuget_source + "'").replace("'REPO'", "'" + windows(copy) + "'").replace("SUITE", suite), encoding='utf-8-sig')
            ...
            counts = re.search(r'POWIAZANIA: ' + r'(\d+) OK / (\d+) BLAD / razem (\d+)', text)
            if not counts:
