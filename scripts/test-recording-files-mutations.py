"""Discriminating checks of recording file history; never edits the source tree.
...
def main():
    ...
    files = 'src/AccessibleMediaController.Windows/MainWindow.RecordingFiles.cs'
    probe = 'src/AccessibleMediaController.Core/Presentation/RecordingPathProbe.cs'
    mutations = [
        ('green', None, None, None),
        ('lose-time', files, 'if (completedUtcTicks > 0 && completedUtcTicks <= DateTime.MaxValue.Ticks)', 'if (completedUtcTicks < 0 && completedUtcTicks <= DateTime.MaxValue.Ticks)'),
        ('lose-history-rename', files, '_state.Radio.RecordingHistory,\n            oldPath,\n            newPath', '_state.Radio.RecordingHistory,\n            newPath,\n            oldPath'),
        ('hide-failed', main_window, 'entry.Outcome == RadioRecordingOutcome.Failed\n                || entry.Path.Length == 0', 'entry.Path.Length == 0'),
        ('skip-activation-probe', main_window, '        RefreshRecordingAvailabilityOnActivation();', '        // mutation: stale cache after return'),
        ('skip-history-open-probe', main_window, '        _recordingPathProbe.Clear();\n        BeginTransientPreview(TransientPreviewKind.RecordedRadioFiles);', '        BeginTransientPreview(TransientPreviewKind.RecordedRadioFiles);'),
        ('drop-unavailable', main_window, '!_recordingPathProbe.IsMissing(item.Source ?? string.Empty)', '_recordingPathProbe.Exists(item.Source ?? string.Empty)'),
        ('skip-normalization', probe, 'return Path.GetFullPath(path.Trim());', 'return path.Trim();'),
        ('hide-interrupted', main_window, '!interruptedPaths.Contains(NormalizeLocalFilePath(item.Source ?? string.Empty))', 'true'),
        ('green-final', None, None, None),
    ]
    receipts = []
    expected_total = None
...
dotnet run --project tests/AccessibleMediaController.Windows.SmokeTests -c Release --no-build --disable-build-servers -- --recording-files-acceptance
exit $LASTEXITCODE
""".replace("'DOTNET'", "'" + args.dotnet_root + "'").replace("'NUGET'", "'" + args.nuget_source + "'").replace("'REPO'", "'" + windows(copy) + "'"), encoding='utf-8-sig')
