...
The Radio scheduler is the next module after `alpha.140` testing. A record stores a stable station identifier plus a safe URL snapshot, start time, time zone, duration, one-off or selected-day recurrence, destination folder, MP3 output and a three-state wake rule: inherit, enable or disable. List mutations and persistence are serialised and committed atomically; restart normalises recurring dates. A late start records only the remaining part of an interval that is still active, while a fully missed occurrence never records its complete duration after the fact. The nearest occurrence requiring wake maintains one current Windows wake timer. Recording runs inside AMC independently of NVDA and reuses the existing resilient stream and MP3-writing pipeline.
