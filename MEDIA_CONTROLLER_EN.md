...
Radio recording is an explicitly started local private-use feature: `Ctrl+Alt+R` writes the decoded stream from that point to WAV under `Music\AMC — Nagrania radia` by default. It never starts automatically, does not apply to TIDAL, Spotify or Apple Music, does not bypass DRM and leaves compliance with applicable law to the user. The first version stops safely when a connection breaks; bounded automatic reconnect remains a later hardening step.
