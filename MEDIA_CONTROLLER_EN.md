...
Stream reception and decoding run away from the window thread and feed a bounded in-memory circular time-shift buffer. Left and Right move inside that buffer, Home selects its oldest retained audio, End returns to live, and the time commands distinguish the current buffer position, delay from live and retained duration. The buffer is not an archive and disappears when the station stops. Bookmarks, percentage or entered-time jumps, and playback-speed controls are not offered for a live broadcast.
