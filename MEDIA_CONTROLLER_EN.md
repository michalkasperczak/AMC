...
**Device presets are not Internet Radio playlists.** In a WiiM or another adapter that declares actual preset support, the planned `Ctrl+P` opens Presets and `Ctrl+Shift+P` assigns or changes one according to the API. The design reserves twelve direct positions corresponding to `1–0`, `-` and `=`, provisionally with `Ctrl+Shift`; implementation requires keyboard-layout testing and configurability. Assigning an Internet Radio station to a WiiM preset belongs to the WiiM device session and does not create a preset in the local Radio session.
