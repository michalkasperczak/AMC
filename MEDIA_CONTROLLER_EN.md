...
The `alpha.100` safeguard preserves a snapshot of the playback context across folder synchronization. If the current file disappears outside AMC, the core searches only for the next available identifier in that snapshot. This applies to Queue, folders, albums, Favorites and playlists, including the return from an automatically entered Queue to its earlier view. A successor is selected but is not started automatically. If no successor exists, the session enters an explicit no-current-item state; Space cannot then play the first catalogue file or an item from another view.
