...
and `Ctrl+Shift+Left/Right` navigates chapter starts. The list can play only a
...
Decision in `alpha.126`: a shortcut must never silently change adapters and then execute a command belonging to another service. `Alt+1–3`, `F5` and `Ctrl+F5` are Local Library layout/source commands; outside Local Files they are intercepted without changing session, hidden from that session's View menu, and not described as available by Keyboard Help. Future radio meanings for `Alt+1–3` require an explicit design decision. Radio playlist import distinguishes a new URL, an existing directory entry outside the Library, and a duplicate already in the Library. The second case promotes the existing object and preserves its richer metadata.

HLS broadcasts carrying video plus AAC audio use an optional FFmpeg child process off the dispatcher. It emits only bounded PCM into the existing time-shift, recording and WASAPI pipeline; arguments bypass the shell, private URLs are not logged, and cancellation closes both pipe and process. The current alpha discovers FFmpeg beside AMC, through `FFMPEG_PATH`, or on `PATH`; final distribution requires a separately updated component with a fixed licence-compatible build. Live checks returned audio from the Jarosław and St John Vianney feeds. A YouTube page is neither HLS nor direct radio and remains a future URL-resolver adapter. Radio bitrate comes from directory metadata, the actual decoder, or a bounded explicit probe of ICY headers, confirmed MP3 frames, ADTS AAC frames, declared HLS audio bandwidth, or an unambiguously audio-only segment. A manifest carrying video can never expose total video bandwidth as radio bitrate. When a protected or malformed server prevents measurement, a maintained compatibility profile for that exact provider may supply a clearly approximate fallback. Source bitrate is never inferred from decoded PCM or recording quality. Detected bitrate and sample rate persist. The native status bar keeps its system role but has no additional accessible name; only its child label exposes dynamic status, so `NVDA+End` does not prepend “Playback status bar”. In Radio the window title starts with the station and may append a current track or programme only when the active stream exposes it. The transient text is bounded, sanitized, never replaces the station name and is cleared before a station switch.
...
| `R` in the Radio player | start or stop a manual capture of the current station |
| `T` in the Radio player or Recording view | finalise the current manual-capture part and start a new file |
| `Shift+R` in Radio | open timed recording and scheduling with optional file splitting |
...
State in `alpha.107`: a Keyboard Help message has the stable order **shortcut → action → active session name**. Context no longer repeats the current view or control type because the key action has already been described in the same message. The context is exactly the user-facing session name from the switcher, for example `Local files`, `WiiM`, `TIDAL` or `Apple Music`. Left Arrow in the local session is described as announcing file size and bitrate; service adapters describe bitrate and other available parameters without promising values the adapter does not expose. The same explicit wording replaces the generic “quick information” entry in the `F1` reference.

State in `alpha.108`: a shared decoder guard covers MP3, MP2, WAV, AAC, M4A, FLAC, WMA, OGG/Vorbis, Opus and AIFF. Before playback it rejects invalid sample rates, channel counts, decoded lengths, absurd durations and an unmistakably impossible duration-to-file-size ratio. Status and accessibility position reads use a non-blocking cached value instead of waiting for a busy decoder. A playback monitor stops the current item after eight seconds without decoder progress, records the source and reason, and quarantines that source until AMC restarts. It never closes AMC or modifies the media file. Quick technical information opens a decoder away from the UI thread with a five-second limit and does not repeatedly probe a source that already timed out. Playback preparation retains its separate 30-second local and two-minute explicit cloud-download limits.

State since `alpha.110`: a local folder is a container, not a media item stored in collections. Enter opens the folder level. Queue, Play Next, Favourites, and playlist commands recursively expand a selected folder into unique, available tracks that are already present in AMC's catalogue. They do not probe storage, read metadata, or force cloud hydration. Input order follows relative paths in the folder. Toggled collections use container semantics: if any expanded track belongs to the collection, the command removes all remaining folder contents from it; if none belongs, it adds all contents. Playing or manually removing one queued file therefore cannot cause the folder command to re-add everything. The batch is one `Ctrl+Z` operation that restores the exact previous partial state. Announcements name the folder and real file count, and an empty folder creates no record. The core also rejects a synthetic `MediaItemKind.Folder` if another interface accidentally forwards it without expansion. `Ctrl+Shift+L` remains a file action, not a registered source-folder action.

State in `alpha.111`: changing decoder position must never occupy the UI thread. A mounted cloud provider may block a position setter while fetching a distant fragment, so the player performs seeks in one background worker associated with the current pipeline. New requests replace the pending target; once earlier access completes, only the newest target is applied. Until completion, the model and status bar return the requested position, letting the next shortcut calculate from the target without losing focus. Track changes, Stop, and shutdown invalidate the worker without applying its result to a new pipeline. The ordinary eight-second read watchdog does not mistake an intentional fetch for a decoder failure; a separate seek timeout is two minutes. Diagnostics record the item, target, and elapsed time for a slow seek. Common streamed Google Drive, iCloud, OneDrive, and Dropbox paths are recognized even without Cloud Files attributes, and recognition never opens the file payload.
...
Ogg routing uses that result: Vorbis goes to the normalised NVorbis path, while Opus or another Ogg stream goes to the system path. NAudio 2.3.0 parses WAV and AIFF directly, including its RF64 fix; MP3 uses Media Foundation with the bounded local NLayer fallback; AAC/MP4, FLAC, WMA/ASF, Matroska/WebM, and other registered formats use Media Foundation and codecs available on that Windows installation. AMC does not install a global codec pack. After an exception from any decoder, the current pipeline is atomically detached and released, so retrying a remote source creates a fresh pipeline and an unambiguously damaged local source can be quarantined until restart. The final sample provider requires complete channel frames; NaN and infinity are zeroed with one diagnostic entry. This check is linear only in the small output buffer and never copies the whole media file.

State in `alpha.114`: cloud classification first obtains `FileAttributeTagInfo` from a handle opened against the reparse point only, then calls `CfGetPlaceholderStateFromAttributeTag`. It requests no payload access and cannot initiate hydration. `PARTIAL` or `PARTIALLY_ON_DISK` means a placeholder, while the `INVALID` sentinel is explicitly excluded from flag tests. This provider-independent route covers every Windows Cloud Files implementation. Sync-root names, virtual-drive volume labels, network shares, and common Google Drive, iCloud, OneDrive, Dropbox, Box, pCloud, MEGA, Proton Drive, Nextcloud, ownCloud, and Sync path segments remain conservative fallbacks. Discovery still never opens payloads. Repeated failures of the same remote source use exponential cooldowns of 10, 30, 90, and at most 300 seconds; a successful start clears the counter and local files never use this mechanism.

The MP3 preflight now separates the declared audio start from the first actually verified frame. ID3 versions outside 2–4 and non-sync-safe size bytes are warned about and cannot control a seek. A valid tag may be hundreds of megabytes: only its ten-byte header is read before a direct seek and a scan of at most 1 MiB. Ordinary MP3 requires two compatible frames; free-format requires three compatible frames at equal spacing to resist false positives. A large tag, warning, or leading junk creates a seekable substream view beginning at the confirmed frame. The view uses 64-bit positions, copies no content, and lets Media Foundation handle files beyond 4 GiB. If that path fails, NLayer remains the last attempt only for a recognised local file up to 512 MiB. Initial RIFF/WAVE/AVI, AIFF, FLAC, Ogg, MP4, and ADTS AAC size declarations are compared with the physical end without walking the complete container. This remains warning-level validation: the decoder may still accept an unusual but supported file.
...
Internet radio is a separate core adapter and uses the same sessions, Favorites, history and transport commands. The `alpha.115` implementation accepts direct streams, M3U/M3U8, PLS and XSPF, presents station metadata and searches the public Radio Browser directory. M3U8 import distinguishes an ordinary station list from an HLS manifest containing `#EXT-X-` tags. Free Radio remains a behavior reference, but neither playback nor its binary engine was moved into the NVDA process.

State of `alpha.117`: a direct MP3 stream may use a modern HTTP response or the legacy Shoutcast/Icecast `ICY 200 OK` status line. The system decoder remains the first choice. Only after it rejects the connection does the bounded compatibility client reconnect, accept a valid audio response, enforce connection and header-size limits, strip ICY metadata blocks, and decode MP3 frames through managed NLayer. There is no station-specific exception list. HTTPS retains normal certificate validation, and unsupported chunked transfer is never guessed.

State of `alpha.120`: the ICY MP3 socket path uses frame-oriented rather than single-packet read semantics. Its input layer fills the requested fragment from consecutive network pieces until the complete portion is available or the connection is genuinely closed. This also covers the boundary between the initial alignment prefix and the continuing stream. NLayer therefore no longer receives a short read that resembles an incomplete final frame. The local regression sends data in 257-byte fragments, and live tests perform multiple consecutive reads from four Polish Radio services. A real disconnection still ends in a controlled failure; the fix creates neither an infinite retry loop nor a full in-memory copy of the broadcast.

State of `alpha.121`: the resolver knows the Polish Radio AAC+→MP3 pairs for Programme 3 `8954→8904` and Radio Chopin `8960→8910`, as well as the Programme 3 HLS host `stream13`. These are explicit same-broadcaster pairs rather than a heuristic over arbitrary ports. A socket check confirmed the working 192 kbps Radio Chopin MP3 stream. At the same time both legacy Programme 3 endpoints returned `ICY 401 Service Unavailable`, so a client cannot repair that outage without another source such as HLS/AAC. Pipeline preparation now reads the first decoded PCM portion before starting WASAPI and raising the started event; this removes a false playing state for headers followed by immediate EOF and pre-fills the time-shift buffer.

State of `alpha.122`: releasing a cancelled pipeline may concurrently interrupt a blocking `BufferedStream` read with `NotSupportedException`. With the cancellation marker set, this is expected completion just like disposal or an I/O error. Without cancellation the same exception still detaches the current pipeline and raises a controlled interrupted-stream message. The capture task leaves no exception for the finalizer and cannot compete with the new station.
...
The **Internet Radio** session owns a local Library and separate Favorites. Directory results never become Favorites automatically. `Insert` in the Library adds a custom URL; `F2` always edits the user-facing station name and stream URL as two independent fields. `Alt+Enter` includes the station name, kind, session, country, language, tags, codec, bitrate, stream URL and homepage when supplied by the directory, but never radio-inapplicable Queue or Play Next state. The body remains read-only text with character and word navigation, while the stream address and homepage also appear in an explicitly labelled link list whose selected item opens through the safe system association on Enter. Internal directory identifiers never enter normal screen-reader output.

Decision in `alpha.129`, extended in `alpha.135`: **Internet Radio** exposes its station Library, Favorites, History, station playlists, search, information, time-shift and recording. It does not expose the track Queue, Play Next, Albums or Bookmarks. Inapplicable actions are removed from the main menu, list and player context menus, search results and the command palette, while the command router also rejects their shortcuts. Legacy radio queue state is neither restored nor persisted. A homogeneous station list omits the `station` kind because the list control already conveys its role and position. A genuine HLS manifest is authoritative and a known legacy MP3 endpoint is only a fallback.

Manual ordering of radio Favorites has two modes. `Alt+Up/Down` moves a contiguous block by one position for small adjustments. `Ctrl+X` records the stable identifiers of the selected stations; the user navigates normally to the destination, and `Ctrl+V` atomically inserts the block before that station. This operation does not touch the file clipboard or stream URLs and performs one list refresh. Filtering must be cleared so hidden rows cannot make the destination ambiguous.

AMC distinguishes **AMC-owned presets in every session** from **device presets** owned by WiiM, BluOS or a future hardware adapter. Every session has twelve independent positions `1–0`, `-` and `=`. `Ctrl+Shift+1–0/-/=` immediately recalls an occupied position in the active session; an empty one changes nothing and explains assignment. `Ctrl+Alt+P` opens a recall-only accessible list where Enter and Space start an occupied preset and can never save or overwrite it. Shift selects multiple slots; `Ctrl+C` copies occupied item names and `Ctrl+Shift+C` copies names plus available paths or addresses while skipping empty slots. `Ctrl+Alt+Shift+P` opens assignment for the current or selected item, announces the first free position, accepts one unmodified slot key and saves with Enter. An occupied position requires selecting the same key again before Enter; another key changes the target and Escape cancels. Delete prepares clearing the selected assignment, also confirmed with Enter. This keeps recall immediate while avoiding the defect of a normal double tap, whose first press would already start the old target. Long press is not used because key repeat, keyboard layout and screen-reader interaction make it less predictable.

On the preset list, pressing an unmodified slot key (`1–0`, minus, or equals) only moves focus to the corresponding position; it neither recalls nor changes the assignment. The list retains positions 1–12, so key `0` points to position 10. Direct recall of an empty position instead uses the physical key label: `Ctrl+Shift+0` announces “Preset 0 empty”. Direct shortcuts are also handled at the Windows key-message boundary so ordinary WPF processing or a screen reader cannot lose the zero key.
...
In the current **Local Files** implementation, a preset can target a file, any visible folder under a registered Library source, an inferred album, or an AMC playlist. A file uses its stable AMC catalogue identifier and starts playing when recalled. A folder stores its full path, while an album or playlist opens its contents without starting audio automatically. The same list and assignment command work in Folders, All files, Custom order, Favorites, search results, and the player. Recall never changes the active session. If a target leaves the active Library or a source folder is detached, the assignment is retained and reported as unavailable so temporary cloud unavailability cannot erase user data.

In the current Radio implementation a preset can point to one station or an AMC station playlist. A station starts immediately, while a playlist opens as an ordinary view and the user chooses a station with Enter. Page Up and Page Down then remain in the playback context from which the station was started. A radio playlist is not a folder that advances automatically: a live stream has no natural end, so changing stations remains an explicit user action.

Stream reception and decoding run away from the window thread and feed a bounded in-memory circular time-shift buffer. Left and Right move inside that buffer, Home selects its oldest retained audio, End returns to live, and the time commands distinguish the current buffer position, delay from live and retained duration. The buffer is not an archive and disappears when the station stops. In the radio player, digits `0–9` seek to `0%, 10%, …, 90%` of the currently retained buffer, including after its oldest data have been overwritten. `0` selects the oldest retained audio, `5` the middle, and `9` a point near live; `End` still returns to live. Seeking preserves pause and the selected playback rate. Percentages are not based on the configured capacity or the full elapsed broadcast time. An empty or preparing buffer produces a message. The existing percentage/time and announcement-muting settings apply; announced time is relative to the retained buffer start. Entered-time/percentage dialogs and bookmarks for the broadcast remain unavailable; playback-rate changes apply to retained audio behind live.

Radio recording is an explicitly started local private-use feature: `R` in the player and `Ctrl+Alt+R` on a list store the selected stream in the configured format and folder. Since `alpha.153`, every manual capture uses a private inaudible pipeline, so changing the audible station does not end it and different stations may be captured concurrently. `T` finalises the current manual-capture part and starts the next file on that same pipeline while preserving a pause; pause markers remain associated with the part in which they were created. Starting with `alpha.201`, each finalised part enters the Local Library immediately, while another `T` during the first five seconds of the fresh part is ignored without stopping the complete capture. `Shift+T` is not a split command. A second command for that station or orderly application shutdown finalises the capture. `Alt+2` opens the transient list of active manual and scheduled captures; it is not restored after restart. The default result is MP3 encoded by Windows Media Foundation at a target 192 kbps. Supported MP3, AAC, OGG and other sources may pass through decoded PCM, while Original mode copies source packets through a second FFmpeg connection. Output first goes to an `.amc-partial` work file and receives its final name atomically only after the encoder closes successfully. An encoder failure removes incomplete data, produces a non-technical accessible message and does not stop listening playback. Recording never starts automatically, does not apply to TIDAL, Spotify or Apple Music, does not bypass DRM and leaves compliance with applicable law to the user.
...
The operational component inventory, mandatory FFmpeg tests and the rules for
separate NVDA add-on packages are maintained in `AKTUALIZACJE_KOMPONENTOW.md`.
...
The `alpha.195` correction treats an explicitly opened `Ctrl+H` list as the visible playback context rather than a hidden playlist: starting an item there makes `Page Up/Page Down` and natural end-of-file continuation follow the current History order. Temporary `Alt+Up/Down` traversal performed inside the player still preserves the context from which the player had previously been opened. Bookmarks and search results remain locators.
...
History semantics in `alpha.104`: `Ctrl+H` opens a persistent per-session list of unique items ordered newest first. It is a full selection list and an item source for copying, Queue, Favorites and playlist management, but it is neither a playback queue nor a manually sorted collection. `Delete` atomically removes only the selected identifiers from that session's History and changes no membership or file. Library removal is available only from views that constitute the Library; Queue and playlist removal likewise affect only the active view. `Shift+Delete` retains separate system semantics and moves an existing local file to the Recycle Bin only after confirmation.
...
Playlist semantics since `alpha.140`: the container and its contents are distinct levels. Enter opens the contents; `Ctrl+Enter` starts the first available item and stores the complete unfiltered playlist as the playback context. Queue, Play next, Favorites and Library commands used on a playlist name are explicitly labelled bulk operations on its contents; they do not silently create a separate “favorite playlist” flag. `Alt+Enter` describes the container. A finite-media list announces total duration, partial metadata is identified as partial, and a station list says “live streams” because it has no finite duration. Queue and Play next remain disabled on Radio containers. Local `Alt+1/2/3` may leave an open playlist for the three Library views.
...
Addition in `alpha.142`: the foreground AMC window also captures exact `Ctrl+Shift+0` in its low-level keyboard hook before the chord reaches WPF. This deliberately handles screen-reader add-ons that consume the shortcut; it is not a global preset and does nothing while AMC is in the background. `Ctrl+0` remains exclusively the session list. Manual recording is toggled by `R` in the Radio player and by `Ctrl+Alt+R` on a station list. `Shift+R` opens a direct timed-recording dialog: its default mode begins an independent recording immediately for the requested number of minutes, while turning that mode off exposes a future start and recurrence. Since `alpha.153`, manual recording is independent from listening, so a station change opens no modal question and does not interrupt capture; the user explicitly stops the recorded station.

Addition in `alpha.143`: general Radio settings contain the recording-folder mode, `MP3`, `M4A/AAC` or `WAV` output, a 96–320 kb/s lossy bitrate and default wake behaviour. An entry retains an empty output path only as explicit inheritance of the general folder; a custom destination is a full path selected with an accessible picker. Likewise, `null` wake means inheritance, but its accessible label must state the current general value. Before capture, a real write probe validates the destination, with this fallback chain: entry folder, general folder, system `Music\AMC — Radio recordings`. No class name, enum or raw value may reach NVDA.

Recording sits after decoding and consumes shared PCM, so the source container does not dictate output format. This architecture covers HLS audio extracted by FFmpeg, system decoders, OGG and BASS; audible manual playback and the schedule's private pipeline share URL resolution and reconnect rules. There is deliberately no HLS “original” mode: copying changing segments, keys, initialisation maps and adaptive variants would be a separate recorder with a different risk profile. The encoder publishes output atomically only after finalisation; an invalid or incomplete working file never masquerades as a recording.

Correction in `alpha.144`: the direct `Shift+R` editor receives an ordered set from the currently visible list rows rather than every session object. This matters because the Radio Browser adapter deliberately retains fetched results in a technical catalogue for reuse. The complete schedule manager expands that scope only with Library, Favorites, the current station and stable references used by existing entries. View scope and selection have a regression test independent of UI Automation.

Active capture state matches a station by stable identifier and, as a fallback, its exact stream URL. Shared list formatting adds one `recording` state after the station name without repeating times or paths on every row. A manual capture retains its start instant, format and destination file in memory; an active schedule also retains actual start, deadline and intended folder. The existing `Alt+Enter` information view exposes these details in a Recording section, and the labels clear atomically after completion or failure.
...
Recording since `alpha.146` distinguishes PCM encoding from source-packet copying. MP3, AAC and FLAC encoders consume decoded PCM, while WAV stores the PCM itself. Original mode opens a separate FFmpeg connection and uses stream copy, so it does not apply another lossy codec; the container follows the detected codec and source kind. HLS is remuxed to an audio-only MPEG transport stream rather than saved as a manifest. The final file is still published atomically from `.amc-partial` only after successful finalisation. Missing FFmpeg, demuxer failure, an empty result or failed shutdown removes the working file and does not stop listening playback. Original capture may create a second connection to the station server and does not promise to preserve non-audio textual metadata.

Recording-folder rule since `alpha.147`: application settings retain exactly one explicit, absolute default-recording-folder path. The interface does not split it into technical “system” and “custom” modes; it exposes the path and a folder picker. A legacy empty value resolves on load to `Music\AMC — Radio recordings`, preserving existing data. A schedule stores an empty path only as the inheritance marker, or a complete entry-specific path as its exception. Its accessible combo must expose only the user-facing labels “Default recording folder” and “Different folder”, never type names, enum values or technical identifiers. Changing the default dynamically affects all inheriting entries, while an entry-specific choice remains independent. An unavailable target still follows the safe fallback policy and must not lose a recording merely because a disk or cloud location was disconnected.

Schedule-editor rule since `alpha.148`: weekdays occupy one Tab stop. Arrow keys navigate the list, Space toggles an independent day state, and every row's accessible name always contains its user-facing day plus “checked” or “not checked”. The checked state must never be conflated with the list's current row. The schedule list similarly toggles `Enabled` with Space without opening the editor; disabling never deletes the record. Manager changes are transactional and reach the main scheduler only after Save. Immediate start means the first capture begins as soon as the entry is accepted, forces the entry enabled, and ignores its first date and time. For recurring entries, that real starting time becomes the time of later occurrences. A per-entry override is labelled “User folder”; internally it remains a complete path, while an empty value continues to mean inheritance.

Schedule time and focus rule since `alpha.149`: a new entry separates **Immediately after saving** from **At the selected time** in one combo. Date and time are native segmented Windows controls: Left and Right select a part, Up and Down change its value, and the control itself preserves a valid calendar date. Duration is a bounded numeric control. The model still stores UTC together with a time-zone identifier, never the control's display text. Tab moves between user-facing functions rather than implementation details of the host. Escape and Enter must also work inside hosted controls. A new entry focuses its station; editing focuses the date; closing the editor restores focus to the selected row in the schedule list. No control may expose a class, record, enum or property name.

Portable Radio data has two layers. A complete backup preserves one user's exact state, while dedicated Favorites and schedule exports are for safe sharing and merging. `PROJEKT_IMPORTU_EKSPORTU_RADIA.md` defines their privacy and compatibility rules.
...
`Alt+R` opens the transient active-recordings view. Escape or Backspace restores
the exact originating Radio view and selection; `Alt+1` always means All
stations and `Alt+2` is unassigned in Radio. `Shift+Space` toggles pause for supported
...
than being one global application value. `Shift+A` opens its accessible selector
...
session interface and does not invoke the pause-on-player-exit rule. Directly
navigating from the player to Queue, Library, Favorites, or another view in the
same session also preserves audio. Radio may therefore continue on a separate
output while the user inspects other data. Only explicit Escape, Shift+F6, or
the return button follows the user's pause-on-exit setting.
...
selection under `Shift+A`. An external-device session such as WiiM will instead show
