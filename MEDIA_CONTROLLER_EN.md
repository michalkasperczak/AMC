...
State in `alpha.109`: a local folder is a container, not a media item stored in collections. Enter opens the folder level. Queue, Play Next, Favourites, and playlist commands recursively expand a selected folder into unique, available tracks that are already present in AMC's catalogue. They do not probe storage, read metadata, or force cloud hydration. Input order follows relative paths in the folder. Toggle state is calculated across all expanded tracks: if not every track belongs to the collection, all are added; if every track belongs, all are removed. The batch is one `Ctrl+Z` operation. Announcements name the folder and real file count, and an empty folder creates no record. The core also rejects a synthetic `MediaItemKind.Folder` if another interface accidentally forwards it without expansion. `Ctrl+Shift+L` remains a file action, not a registered source-folder action.
