...
Radio recording is an explicitly started local private-use feature: in the current interface `R` stores the decoded stream from that point in the configured format and folder. From `alpha.116`, the default result is MP3 encoded by Windows Media Foundation at a target 192 kbps. This is not a copy of the original broadcast segments: supported MP3, AAC, OGG and other sources pass through one decoded PCM path so an HLS break or source-codec change cannot leave an inconsistent container. Output first goes to an `.amc-partial` work file and receives its final name atomically only after the encoder closes successfully. Stop, station changes and orderly application shutdown finalise it; an encoder failure removes incomplete data, produces a non-technical accessible message and does not stop radio playback. Recording never starts automatically, does not apply to TIDAL, Spotify or Apple Music, does not bypass DRM and leaves compliance with applicable law to the user.
