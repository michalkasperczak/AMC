...
public partial class MainWindow : AccessibleWindow, IAnnouncementSink, IApplicationActions
{
    ...
    private const string WiiMNetworkStreamExternalId = "wiim-network-stream";
    ...
    private readonly SemaphoreSlim _wiiMDeviceOperationGate = new(1, 1);
    private bool _wiiMRefreshInProgress;
    private DateTime _nextWiiMPlayerRefreshUtc = DateTime.MinValue;
    ...
    private readonly DispatcherTimer _podcastRefreshTimer;
    ...
    public MainWindow(PersistedState state, ConfigurationStore store)
    {
        ...
        _podcastRefreshTimer = new DispatcherTimer(DispatcherPriority.Background)
        {
        ...
        _podcastOutput.ConfigureAudioProcessingResolver(GetEffectivePodcastAudioSettings);
    ...
    private void NormalizePodcastNavigationAtStartup()
    {
        ...
        {
            _state.SessionNavigation.Sessions["podcasts"] = new SessionNavigationState
            {
                CurrentView = "Biblioteka"
            };
            return;
    ...
    private void ReturnFromPlayerToList()
    {
        ...
        ApplyPendingPodcastReloadIfSafe();
        RestoreFilterForCurrentView(navigation);
    ...
    private void ApplyPendingPodcastReloadIfSafe()
    {
        if (!_podcastReloadPending
            || _playerViewActive
            || !string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal))
        {
            return;
    ...
    private void UpdateWiiMPlayerView(MediaItem deviceItem, bool updateAccessibleName)
    {
        ...
        var artist = secondary.Length > 0
            ...
            : !string.IsNullOrWhiteSpace(playback?.Source)
                ? playback.Source
                : "Urządzenie WiiM";
        var state = playback?.State ?? "stan jeszcze nieodświeżony";
        if (playback?.Muted == true) state += ", wyciszone";
        if (playback is not null) state += $", głośność {playback.Volume}%";

        PlayerTitleText.Text = title;
        PlayerArtistText.Text = artist;
        PlayerSessionText.Text = $"WiiM, {deviceItem.Title}";
        PlayerStateText.Text = state;
        PlayerSpeedText.Visibility = Visibility.Collapsed;
        ...
        var seekVisibility = playback?.Duration > TimeSpan.Zero
            ? Visibility.Visible
            : Visibility.Collapsed;
        PlayerSeekTimeButton.Visibility = seekVisibility;
        PlayerSeekPercentButton.Visibility = seekVisibility;
        PlayerTimeText.Text = playback is null
            ? "Stan czasu nieznany"
            : playback.Duration > TimeSpan.Zero
                ? $"{CommandRouter.FormatTime(playback.Position)} z {CommandRouter.FormatTime(playback.Duration)}"
                : CommandRouter.FormatTime(playback.Position);
        RadioRecordingButton.Visibility = Visibility.Collapsed;
        PlayerHelpText.Text = PlayerKeyboardHelpText();
        var playing = string.Equals(playback?.State, "odtwarzanie", StringComparison.OrdinalIgnoreCase);
        var action = playing ? "Wstrzymaj" : "Odtwórz";
    ...
    private void PlayerUiTimer_Tick(object? sender, EventArgs e)
    {
        ...
        if (_playerViewActive
            ...
            && DateTime.UtcNow >= _nextWiiMPlayerRefreshUtc)
        {
    ...
    private async Task RecognizeCurrentRadioTrackAsync(bool automatic)
    {
        ...
        try
        {
            IReadOnlyList<RadioRecognitionInput> inputs;
            if (automatic)
            {
                inputs = CaptureAutomaticRadioRecognitionInputs();
            }
            else
            {
                var station = CurrentPlayedRadioStation();
                if (station is null)
                {
                    Announce("Najpierw uruchom stację radiową");
                    return;
                }
                if (!_radioOutput.TryGetRecentPlaybackAudio(
                        RadioRecognitionSampleDuration,
            ...
            if (inputs.Count == 0)
            {
                if (automatic && HasPotentialAutomaticRadioRecognitionSource())
                    ScheduleNextRadioRecognition(RadioRecognitionRetryDelay);
                else if (automatic)
            ...
            {
                recognizedAny |= await RecognizeRadioInputAsync(input, automatic);
                if (_isClosing) return;
            }
            if (automatic && !recognizedAny)
                ScheduleNextRadioRecognition(RadioRecognitionRetryDelay);
        }
    ...
    private MediaItem? CurrentPlayedRadioStation()
    {
        ...
        return loadedId is null || radioSession?.IsPlaying != true
            ? null
            : radioSession.Items.FirstOrDefault(item =>
                string.Equals(item.Id, loadedId, StringComparison.Ordinal));
    }

    private IReadOnlyList<RadioRecognitionInput> CaptureAutomaticRadioRecognitionInputs()
    {
        var result = new List<RadioRecognitionInput>();
        var keys = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var scope = _state.Radio.AutomaticTrackRecognitionScope;

        if (RadioRecognitionScopeRules.IncludesCurrentStation(scope))
        {
            var station = CurrentPlayedRadioStation();
            if (station is not null
                && _radioOutput.TryGetRecentPlaybackAudio(
                    RadioRecognitionSampleDuration,
                    out var snapshot)
                && snapshot is not null)
            {
                AddRadioRecognitionInput(
                    result,
                    keys,
                    station.Id,
                    station.Title,
                    station.Source ?? station.PublicUri ?? string.Empty,
                    snapshot);
            }
        }

        if (!RadioRecognitionScopeRules.IncludesRecordingStations(scope)) return result;

        foreach (var active in _activeManualRadioRecordings.Values.OrderBy(item => item.RequestedUtc))
        {
            if (active.Control.IsPaused
                || !active.Control.TryGetRecentAudio(RadioRecognitionSampleDuration, out var snapshot)
                || snapshot is null) continue;
            AddRadioRecognitionInput(
                result,
                keys,
                active.StationId,
        ...
        foreach (var active in _activeScheduledRadioRecordings.Values.OrderBy(item => item.StartedUtc))
        {
            if (active.Control.IsPaused
                || !active.Control.TryGetRecentAudio(RadioRecognitionSampleDuration, out var snapshot)
                || snapshot is null) continue;
    ...
    private static void AddRadioRecognitionInput(
        ICollection<RadioRecognitionInput> result,
        ISet<string> keys,
        string stationId,
        string stationName,
        string streamUrl,
    ...
    {
        var key = string.IsNullOrWhiteSpace(stationId)
            ? $"url:{streamUrl.Trim()}"
            : $"id:{stationId.Trim()}";
        if (!keys.Add(key)) return;
        result.Add(new RadioRecognitionInput(
            stationId.Trim(),
            string.IsNullOrWhiteSpace(stationName) ? "Nieznana stacja" : stationName.Trim(),
            snapshot));
    }

    private bool HasPotentialAutomaticRadioRecognitionSource()
    {
        var scope = _state.Radio.AutomaticTrackRecognitionScope;
        return (RadioRecognitionScopeRules.IncludesCurrentStation(scope)
                && CurrentPlayedRadioStation() is not null)
               || (RadioRecognitionScopeRules.IncludesRecordingStations(scope)
                   && (_activeManualRadioRecordings.Values.Any(active => !active.Control.IsPaused)
                       || _activeScheduledRadioRecordings.Values.Any(active => !active.Control.IsPaused)));
    }

    private async Task<bool> RecognizeRadioInputAsync(
        RadioRecognitionInput input,
        bool automatic)
    {
        DiagnosticLog.Info(
            "recognition",
            $"Rozpoczęto rozpoznawanie z {input.Snapshot.Duration.TotalSeconds:0.0} sekundy bufora stacji {input.StationName}.");
        ...
        if (!result.Success)
        {
            DiagnosticLog.Info(
                "recognition",
                $"{input.StationName}: {result.Error ?? "Nie rozpoznano utworu."}");
        ...
        var duplicate = _state.Radio.RecognizedTracks
            ...
            .FirstOrDefault(entry =>
                string.Equals(entry.StationId, input.StationId, StringComparison.Ordinal)
        ...
        if (duplicate is null)
        {
            ...
            QueueStateSave();
            DiagnosticLog.Info("recognition", $"Rozpoznano {label} na stacji {input.StationName}.");
            if (ShouldAnnounceRadioRecognitionResult(
                    ...
                    IsActive))
            {
                AnnounceEssential(automatic
                    ? $"Rozpoznano w stacji {input.StationName}: {label}"
                    : $"Rozpoznano: {label}");
            }
        }
    ...
    private void RefreshRadioRecognitionScheduleAfterSourceChange()
    {
        if (!_radioRecognitionMonitoring || !HasPotentialAutomaticRadioRecognitionSource())
        {
            _nextRadioRecognitionUtc = DateTime.MaxValue;
            return;
        }

        var initial = DateTime.UtcNow.Add(RadioRecognitionInitialDelay);
        if (_nextRadioRecognitionUtc == DateTime.MaxValue || _nextRadioRecognitionUtc > initial)
            _nextRadioRecognitionUtc = initial;
    }
    ...
    private void ToggleRadioRecognitionMonitoring()
    {
        ...
        if (enabled && HasPotentialAutomaticRadioRecognitionSource())
        {
        ...
        QueueStateSave();
        UpdateFileMenuForCurrentSession();
        var scope = RadioRecognitionScopeRules.GetLabel(
            _state.Radio.AutomaticTrackRecognitionScope);
        AnnounceEssential(enabled
            ? HasPotentialAutomaticRadioRecognitionSource()
                ? $"Włączono i zapamiętano obserwowanie rozpoznawania: {scope}"
                : $"Włączono i zapamiętano obserwowanie rozpoznawania: {scope}. Rozpoznawanie rozpocznie się, gdy pojawi się odpowiednie źródło"
    ...
    private string BuildWiiMPlaybackStatusText(MediaItem deviceItem)
    {
        ...
        parts.Add($"głośność {snapshot.Playback.Volume}%");
        if (snapshot.Playback.Duration > TimeSpan.Zero)
        {
            parts.Add($"{CommandRouter.FormatTime(snapshot.Playback.Position)} z {CommandRouter.FormatTime(snapshot.Playback.Duration)}");
        }
        return string.Join(", ", parts);
    }
    ...
    {
        if (TryHandleArtistSectionCommand(CommandIds.ManagePlaylists)) return;
        ShowPlaylistManager(ActionItems, ActionSession);
    }
    ...
    private bool CommandVisibleInPalette(string commandId)
    {
        ...
        if (commandId is CommandIds.ManageWiiMDevices
            ...
            or CommandIds.ViewWiiMNetworkStreams
    ...
    {
        if (TryHandleArtistSectionCommand(CommandIds.ItemProperties)) return;
            ...
            : item.Kind is MediaItemKind.Podcast or MediaItemKind.Episode
                ? new[]
                {
                    string.IsNullOrWhiteSpace(item.PublicUri)
                        ? null
                        : new InformationLink("Otwórz stronę", item.PublicUri),
                    string.IsNullOrWhiteSpace(item.Source)
                        ? null
                        : new InformationLink(
                            item.Kind == MediaItemKind.Podcast
                                ? "Otwórz adres kanału RSS lub Atom"
                                : "Otwórz bezpośredni plik audio",
                            item.Source)
                }.Where(link => link is not null).Select(link => link!).ToArray()
                : [];
    ...
    {
        CapturePodcastState();
        var episode = FindPodcastEpisodeSettings(item);
        var subscription = FindPodcastSubscriptionSettings(item);
        if (subscription is null || item.Kind == MediaItemKind.Episode && episode is null)
        {
            Announce("Nie można odnaleźć ustawień tego podcastu");
            RestoreItemActionFocus();
            return;
        }

        var target = episode is null
            ? ItemPlaybackOptionsTarget.Podcast
            : ItemPlaybackOptionsTarget.PodcastEpisode;
        var dialog = new ItemPlaybackOptionsWindow(
            item.Title,
            episode?.ResumePositionMode ?? subscription.ResumePositionMode,
            episode is null ? subscription.PlaybackRateOverride : episode.PlaybackRateOverride,
            episode is null ? subscription.LoudnessNormalizationOverride : episode.LoudnessNormalizationOverride,
            episode is null ? subscription.SmoothTrackTransitionsOverride : episode.SmoothTrackTransitionsOverride,
            episode is null
        ...
        if (episode is null)
        {
            subscription.ResumePositionMode = dialog.SelectedResumePositionMode;
            subscription.PlaybackRateOverride = dialog.SelectedPlaybackRateOverride;
            subscription.LoudnessNormalizationOverride = dialog.SelectedLoudnessNormalizationOverride;
        ...
        {
            episode.ResumePositionMode = dialog.SelectedResumePositionMode;
            episode.PlaybackRateOverride = dialog.SelectedPlaybackRateOverride;
            episode.LoudnessNormalizationOverride = dialog.SelectedLoudnessNormalizationOverride;
        ...
        if (podcasts is not null && podcasts.HasCurrentItem)
        {
            if (!ShouldRememberPodcastPosition(podcasts.CurrentItem))
                podcasts.ClearRememberedPosition(podcasts.CurrentItem.Id);
            podcasts.ApplyPlaybackRateForCurrentItem();
            _podcastOutput.ConfigureAudioProcessing(
                GetEffectivePodcastAudioSettings(podcasts.CurrentItem));
        }
        QueueStateSave(announceFailure: true);
        if (_playerViewActive) UpdatePlayerView(true);
        UpdatePlaybackStatusBar();
        UpdateFileMenuForCurrentSession();
        Announce(episode is null
            ? $"Zapisano opcje podcastu: {subscription.Title}"
            : $"Zapisano opcje odcinka: {episode.Title}");
        RestoreItemActionFocus();
    }
    ...
    private string BuildItemPropertiesText(MediaItem item)
    {
        ...
        {
            return BuildWiiMDevicePropertiesText(item);
        }
        ...
        if (item.Kind is MediaItemKind.Podcast or MediaItemKind.Episode
            && string.Equals(session.Id, "podcasts", StringComparison.Ordinal))
        {
            return BuildPodcastPropertiesText(item);
        }
    ...
    private string BuildWiiMDevicePropertiesText(MediaItem item)
    {
        var device = _state.WiiM.Devices.FirstOrDefault(candidate =>
            string.Equals(candidate.Id, item.Id, StringComparison.OrdinalIgnoreCase));
        if (device is null) return $"Urządzenie WiiM{Environment.NewLine}Nazwa: {item.Title}";
        _wiiMSnapshots.TryGetValue(device.Id, out var snapshot);
        var lines = new List<string>
        {
            ...
            $"Dostępność: {(snapshot is null ? "stan jeszcze nieodświeżony albo urządzenie niedostępne" : "dostępne")}",
            string.IsNullOrWhiteSpace(device.Firmware) ? string.Empty : $"Oprogramowanie urządzenia: {device.Firmware}"
        };
        if (snapshot is not null)
        {
            lines.Add($"Stan: {snapshot.Playback.State}");
            if (!string.IsNullOrWhiteSpace(snapshot.Playback.Source)) lines.Add($"Źródło: {snapshot.Playback.Source}");
            if (!string.IsNullOrWhiteSpace(snapshot.Track.Title)) lines.Add($"Tytuł: {snapshot.Track.Title}");
            if (!string.IsNullOrWhiteSpace(snapshot.Track.Artist)) lines.Add($"Wykonawca: {snapshot.Track.Artist}");
            if (!string.IsNullOrWhiteSpace(snapshot.Track.Album)) lines.Add($"Album: {snapshot.Track.Album}");
            lines.Add($"Głośność urządzenia: {snapshot.Playback.Volume}%");
            lines.Add($"Wyciszenie urządzenia: {(snapshot.Playback.Muted ? "tak" : "nie")}");
            if (snapshot.Playback.Duration > TimeSpan.Zero)
            {
                lines.Add($"Pozycja: {CommandRouter.FormatTime(snapshot.Playback.Position)} z {CommandRouter.FormatTime(snapshot.Playback.Duration)}");
            }
    ...
    private string BuildPodcastPropertiesText(MediaItem item, bool includeDescription = true)
    {
        ...
        var episodeSettings = _state.Podcasts.Episodes.FirstOrDefault(candidate =>
            string.Equals(candidate.Id, item.Id, StringComparison.Ordinal));
    ...
    private static int? EstimatePodcastBitrateKbps(PodcastEpisodeSettings episode)
    {
        if (episode.MediaLength is not > 0 || episode.DurationTicks <= 0) return null;
        return LocalAudioFileDiscovery.EstimateBitrateKbps(
            episode.MediaLength.Value,
            TimeSpan.FromTicks(episode.DurationTicks));
    }

    private static string? FormatPodcastCodec(string? mediaType) => mediaType?.Trim().ToLowerInvariant() switch
    {
    ...
    private string BuildQuickMediaInformation(MediaItem item)
    {
        ...
        var podcastEpisode = item.Kind == MediaItemKind.Episode
    ...
    private bool TryGetWiiMPlayableUri(MediaItem? item, out string mediaUrl)
    {
        mediaUrl = string.Empty;
        if (item?.Kind is not (MediaItemKind.Station
            or MediaItemKind.Episode
            or MediaItemKind.Track
            or MediaItemKind.Playlist))
        {
            return false;
        }
        if (WiiMPlaybackUriPolicy.TryNormalize(item.Source, out mediaUrl)) return true;
        return item.Kind == MediaItemKind.Episode
            && WiiMPlaybackUriPolicy.TryNormalize(
                FindPodcastEpisodeSettings(item)?.MediaUrl,
                out mediaUrl);
    }

    private WiiMDeviceSettings? ResolveWiiMPlaybackTarget()
    {
        var selected = _state.WiiM.Devices.FirstOrDefault(device =>
            string.Equals(device.Id, _state.WiiM.SelectedDeviceId, StringComparison.OrdinalIgnoreCase));
    ...
    private async Task OpenCurrentItemOnWiiMAsync(MediaItem? requestedItem = null)
    {
        ...
        var device = ResolveWiiMPlaybackTarget();
        if (device is null)
        {
            Announce(_state.WiiM.Devices.Count == 0
                ? "Brak zapisanego urządzenia WiiM. Przejdź do sesji WiiM i naciśnij Ctrl+F5"
                : "Wybierz aktywne urządzenie w sesji WiiM przed wysłaniem materiału");
            return;
        }
        ...
        try
        {
            ...
            catch (Exception exception) when (IsWiiMConnectionFailure(exception))
            {
                DiagnosticLog.Warning(
                    "wiim-play-url",
                    $"Wysłano adres do urządzenia, ale nie potwierdzono nowego stanu: {exception.GetType().Name}.");
            }
        }
        ...
        catch (Exception exception) when (IsWiiMConnectionFailure(exception))
        {
            DiagnosticLog.Warning(
                "wiim-play-url",
                $"Nie udało się otworzyć adresu na urządzeniu {device.Id}: {exception.GetType().Name}.");
            AnnounceEssential($"Nie udało się otworzyć elementu na urządzeniu {device.DisplayName}");
        ...
        if (_isClosing) return;
        var session = SelectSessionBrowserItem("wiim", device.Id);
        if (session is null)
        {
            AnnounceEssential("Wysłano materiał do WiiM, ale nie można otworzyć sesji urządzenia");
            return;
        }
        _state.WiiM.SelectedDeviceId = device.Id;
        QueueStateSave(announceFailure: true);
        _playerFocusContextPrefix = snapshot is null
            ? $"Wysłano {item.Title} do {device.DisplayName}; urządzenie nie potwierdziło jeszcze stanu"
            : $"Otwarto {item.Title} na {device.DisplayName}";
        ShowPlayerView();
    }

    private void RestoreWiiMFocusAfterExternalCommand()
    {
        Activate();
        if (_playerViewActive) FocusPlayerView();
        else RestoreMediaListFocusAfterRefresh();
    ...
    private void AddWiiMNetworkStream()
    {
        ...
        if (!WiiMPlaybackUriPolicy.TryNormalize(dialog.StreamUrl, out var normalizedUrl))
        {
            AnnounceEssential("Nie można zapisać tego adresu strumienia WiiM");
            RestoreItemActionFocus();
            return;
        }

        var duplicate = _state.WiiM.NetworkStreams.FirstOrDefault(stream =>
            string.Equals(stream.StreamUrl, normalizedUrl, StringComparison.OrdinalIgnoreCase));
        if (duplicate is not null)
        {
            duplicate.Name = dialog.StationName;
            ShowWiiMNetworkStreams(duplicate.Id, $"Zaktualizowano strumień: {duplicate.Name}");
            return;
        }

        var saved = new WiiMNetworkStreamSettings
        {
            Id = $"wiim:stream:{Guid.NewGuid():N}",
            Name = dialog.StationName,
            StreamUrl = normalizedUrl,
            AddedUtcTicks = DateTime.UtcNow.Ticks
        };
        _state.WiiM.NetworkStreams.Add(saved);
        ShowWiiMNetworkStreams(saved.Id, $"Dodano strumień WiiM: {saved.Name}");
    }

    private async void ImportWiiMNetworkStreams()
    {
        var dialog = new OpenFileDialog
        {
        ...
        var existingUrls = _state.WiiM.NetworkStreams
            .Select(stream => stream.StreamUrl)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var added = new List<WiiMNetworkStreamSettings>();
        foreach (var station in import.Stations)
        {
            if (!existingUrls.Add(station.StreamUrl)) continue;
            var stream = new WiiMNetworkStreamSettings
            {
                Id = $"wiim:stream:{Guid.NewGuid():N}",
                Name = station.Name,
            ...
            _state.WiiM.NetworkStreams.Add(stream);
            added.Add(stream);
        }

        if (added.Count == 0)
        {
            Announce(import.SkippedEntries > 0
                ? "Playlista nie zawiera nowych prawidłowych strumieni WiiM"
                : "Wszystkie strumienie z tej playlisty są już zapisane");
            RestoreItemActionFocus();
            return;
        }
    ...
    private void UpdateFileMenuForCurrentSession()
    {
        ...
        var openOnWiiMAvailable = ActionItems.Count == 1
            && TryGetWiiMPlayableUri(ActionItem, out _);
        OpenOnWiiMMainMenuItem.Visibility = openOnWiiMAvailable
            ? Visibility.Visible
            : Visibility.Collapsed;
        if (openOnWiiMAvailable)
        {
            MenuAccessibility.SetPresentation(
                OpenOnWiiMMainMenuItem,
                ResolveWiiMPlaybackTarget() is { } playbackTarget
                    ? $"Otwórz element w {playbackTarget.DisplayName}"
                    : "Otwórz element w WiiM");
        }
        ...
        var currentLocalItem = local
            ...
            && TryGetLocalPath(_sessions.Current.CurrentItem.Source, out _)
                ? _sessions.Current.CurrentItem
                : null;
        var currentPodcastItem = podcasts
            && _playerViewActive
            && _sessions.Current.HasCurrentItem
            && _sessions.Current.CurrentItem.Kind is MediaItemKind.Podcast or MediaItemKind.Episode
                ? _sessions.Current.CurrentItem
                : null;
        ...
        else if (currentPodcastItem is not null)
        {
            MenuAccessibility.SetPresentation(
                PlaybackCurrentItemAudioOptionsMenuItem,
                currentPodcastItem.Kind == MediaItemKind.Podcast
                    ? "Zmień opcje bieżącego podcastu"
        ...
        PlaybackCurrentBroadcastInformationMenuItem.Visibility = radio || wiiM
            ? Visibility.Visible
            : Visibility.Collapsed;
        var tidalRefresh = tidal && _tidalContainerViews.ContainsKey(_currentView);
        RefreshLocalLibraryMenuItem.Visibility = local || podcasts || wiiM || tidalRefresh ? Visibility.Visible : Visibility.Collapsed;
        if (tidalRefresh)
        {
            MenuAccessibility.SetPresentation(RefreshLocalLibraryMenuItem, "Odśwież zawartość TIDAL");
        }
        else if (wiiM)
        {
            MenuAccessibility.SetPresentation(RefreshLocalLibraryMenuItem, "Odśwież urządzenia WiiM");
        }
        else if (podcasts)
        {
            MenuAccessibility.SetPresentation(
        ...
        RenameLibraryItemMainMenuItem.Visibility = local || radio || podcasts
            || wiiM && IsWiiMNetworkStream(ActionItem)
            ? Visibility.Visible
            : Visibility.Collapsed;
        MenuAccessibility.SetPresentation(
            LibraryViewMenuItem,
        ...
        AutomationProperties.SetAcceleratorKey(
            LibraryViewMenuItem,
            "Ctrl+L");
        LibraryViewMenuItem.Visibility = wiiM ? Visibility.Collapsed : Visibility.Visible;
        MenuAccessibility.SetPresentation(
        ...
        BookmarksViewMenuItem.Visibility = radio || wiiM ? Visibility.Collapsed : Visibility.Visible;
        BrowserPlaybackButton.Content = wiiM ? "_Wybierz urządzenie i otwórz sterowanie" : "Odtwórz lub _wstrzymaj";
        AutomationProperties.SetName(
            BrowserPlaybackButton,
    ...
    private void LoadPersistedWiiM()
    {
        _wiiMItems.Clear();
        foreach (var device in _state.WiiM.Devices)
        {
            _wiiMItems.Add(BuildWiiMMediaItem(
                device,
                _wiiMSnapshots.GetValueOrDefault(device.Id)));
        }
    ...
    private static MediaItem BuildWiiMMediaItem(
        ...
        WiiMDeviceSnapshot? snapshot) => new()
    {
        Id = device.Id,
        Title = device.DisplayName,
        Artist = snapshot?.PlaybackSummary ?? "stan jeszcze nieodświeżony",
        Kind = MediaItemKind.Device,
        Source = device.Address,
        ExternalId = device.Id,
        Duration = snapshot?.Playback.Duration ?? TimeSpan.Zero,
        IsInLibrary = true,
    ...
    private void CapturePodcastState()
    {
        ...
        _state.Podcasts.CurrentItemId = session?.HasCurrentItem == true
            ? session.CurrentItem.Id
            : null;
        if (session is not null)
        {
            if (UsesSystemDefaultOutput("podcasts")) _state.Podcasts.Volume = session.Volume;
            if (!session.HasCurrentItem || GetPodcastPlaybackRateOverride(session.CurrentItem) is null)
                _state.Podcasts.PlaybackRate = session.PlaybackRate;
        }
    }

    private void AddPodcastSubscription()
    {
    ...
    private async void ImportPodcastOpml()
    {
        ...
        var picker = new OpenFileDialog
        {
            ...
            Filter = "Pliki OPML (*.opml;*.xml)|*.opml;*.xml|Wszystkie pliki (*.*)|*.*",
            Multiselect = false,
            CheckFileExists = true
        };
        ...
        IReadOnlyList<PodcastOpmlEntry> entries;
        try
        {
            var info = new FileInfo(picker.FileName);
            if (info.Length > PodcastOpmlParser.MaximumXmlCharacters)
                throw new InvalidDataException("Plik OPML jest zbyt duży.");
            entries = PodcastOpmlParser.Parse(await File.ReadAllTextAsync(picker.FileName));
        }
        catch (Exception exception) when (exception is IOException
            or UnauthorizedAccessException
            or InvalidDataException
            or System.Xml.XmlException)
        {
            AnnounceEssential($"Nie można odczytać OPML: {exception.Message}");
            RestoreMediaListFocusAfterRefresh();
            return;
        }
        ...
        if (entries.Count == 0)
        {
            AnnounceEssential("Plik OPML nie zawiera prawidłowych adresów kanałów RSS ani Atom");
            RestoreMediaListFocusAfterRefresh();
            return;
        }

        var selection = new PodcastOpmlImportWindow(entries) { Owner = this };
        ...
        StatusText.Text = $"Importowanie podcastów: {selection.SelectedEntries.Count}";
        CapturePodcastState();
        using var gate = new SemaphoreSlim(4);
        var tasks = selection.SelectedEntries.Select(async entry =>
        {
            await gate.WaitAsync();
            try
            {
                return (Entry: entry, Feed: await _podcastFeedClient.FetchAsync(entry.FeedUri, _podcastCancellation.Token), Error: (Exception?)null);
            }
            catch (Exception exception) when (exception is HttpRequestException
                or InvalidDataException
                or System.Xml.XmlException
            ...
            {
                return (Entry: entry, Feed: (PodcastFeedDocument?)null, Error: exception);
            }
            finally
            {
                gate.Release();
        ...
        if (_isClosing) return;
        var imported = 0;
        foreach (var item in fetched.Where(item => item.Feed is not null))
        {
    ...
    {
        if (!TryResolveCurrentPodcastSubscription(out var subscription))
        {
    ...
    {
        if (_isClosing || Volatile.Read(ref _podcastRefreshInProgress) != 0) return;
        var now = DateTime.UtcNow;
        ...
        await RefreshPodcastSubscriptionsAsync(due, returnToLibrary: false, automatic: true);
    }
        ...
        bool automatic = false)
    {
        ...
        try
        {
            ...
            {
                // Replacing tens of thousands of episode objects while the user
                // is inside the player can stall UI Automation and destroy the
                // playback context under NVDA. Keep the refreshed persistent
                // model and apply its browser snapshot only after a podcast list
                // is visible again.
                _podcastReloadPending = true;
            }
            if (_isClosing) return;
        ...
        {
            Volatile.Write(ref _podcastRefreshInProgress, 0);
        }
    }

    private void ReloadPodcastSessionItems()
    {
        _podcastItems.Clear();
        LoadPersistedPodcasts();
        var session = _sessions.FindSession("podcasts");
        if (session is null) return;
        session.ReplaceItems(_podcastItems);
        foreach (var episode in _state.Podcasts.Episodes.Where(episode => episode.ResumePositionTicks > 0))
        {
            var episodeItem = _podcastItems.FirstOrDefault(item =>
                string.Equals(item.Id, episode.Id, StringComparison.Ordinal));
            if (episodeItem is null || !ShouldRememberPodcastPosition(episodeItem)) continue;
            session.SetRememberedPosition(episode.Id, TimeSpan.FromTicks(episode.ResumePositionTicks));
        }
        ...
        bool restoreListFocus)
    {
        ...
        try
        {
            destinationFolder = saveAs
                ? PodcastDownloadFolderResolver.ResolveDialogInitialFolder(
                    ResolveConfiguredPodcastDownloadFolder(episodes[0]))
                : string.Empty;
        }
        catch (Exception exception) when (exception is IOException
            or UnauthorizedAccessException
        ...
        for (var index = 0; index < episodes.Length; index++)
        {
            ...
            try
            {
                ...
                var itemDestinationFolder = saveAs
                    ? destinationFolder
                    : PodcastDownloadFolderResolver.Resolve(
                        ResolveConfiguredPodcastDownloadFolder(episode));
                var destinationPath = saveAsPath ?? PodcastDownloadNaming.UniquePath(
                    itemDestinationFolder,
    ...
    private string? ResolveConfiguredPodcastDownloadFolder(PodcastEpisodeSettings episode)
    {
        ...
        return PodcastPlaybackSettingsResolver.ConfiguredDownloadFolder(
            _state.Podcasts.DownloadsFolder,
            subscription);
    }
    ...
    private bool TryResolveCurrentPodcastSubscription(out PodcastSubscriptionSettings subscription)
    {
        ...
        if (TryGetPodcastIdFromView(_currentView, out var viewPodcastId))
            subscriptionId = viewPodcastId;
        else if (SelectedItem?.Kind == MediaItemKind.Podcast)
            subscriptionId = SelectedItem.Id;
        else if (SelectedItem?.Kind == MediaItemKind.Episode)
            subscriptionId = SelectedItem.ExternalId;

        subscription = _state.Podcasts.Subscriptions.FirstOrDefault(candidate =>
    ...
    private PodcastEpisodeSettings? FindPodcastEpisodeSettings(MediaItem item) =>
        item.Kind == MediaItemKind.Episode
            ? _state.Podcasts.Episodes.FirstOrDefault(episode =>
                string.Equals(episode.Id, item.Id, StringComparison.Ordinal))
            : null;

    private PodcastSubscriptionSettings? FindPodcastSubscriptionSettings(MediaItem item)
    {
        var subscriptionId = item.Kind == MediaItemKind.Podcast
            ? item.Id
            : FindPodcastEpisodeSettings(item)?.SubscriptionId ?? item.ExternalId;
        return _state.Podcasts.Subscriptions.FirstOrDefault(subscription =>
            string.Equals(subscription.Id, subscriptionId, StringComparison.Ordinal));
    }

    private bool ShouldRememberPodcastPosition(MediaItem item)
    {
        var episode = FindPodcastEpisodeSettings(item);
        return item.Kind == MediaItemKind.Episode
    ...
    private double? GetPodcastPlaybackRateOverride(MediaItem item) =>
        PodcastPlaybackSettingsResolver.PlaybackRateOverride(
            FindPodcastEpisodeSettings(item),
            FindPodcastSubscriptionSettings(item));

    private PlaybackAudioSettings GetEffectivePodcastAudioSettings(MediaItem item) =>
        PodcastPlaybackSettingsResolver.ResolveAudio(
            _state.Settings.Audio,
            FindPodcastEpisodeSettings(item),
            FindPodcastSubscriptionSettings(item));
    ...
    private void RebuildCore()
    {
        ...
        var previousPodcasts = _sessions?.FindSession("podcasts");
        var previousPodcastItemId = previousPodcasts?.HasCurrentItem == true
            ? previousPodcasts.CurrentItem.Id
            : null;
        var previousPodcastWasPlaying = previousPodcasts?.IsPlaying == true;
        var previousWiiM = _sessions?.FindSession("wiim");
        ...
        var (podcasts, _) = _sessions.AddOrUpdateTransientSession(
            ...
            _podcastOutput,
            6,
            ShouldRememberPodcastPosition,
            GetPodcastPlaybackRateOverride,
        ...
        if (podcasts.HasItems)
        {
                         ...
                         episode.ResumePositionTicks > 0))
            {
                var episodeItem = _podcastItems.FirstOrDefault(item =>
                    string.Equals(item.Id, episode.Id, StringComparison.Ordinal));
                if (episodeItem is null || !ShouldRememberPodcastPosition(episodeItem)) continue;
                podcasts.SetRememberedPosition(
                    episode.Id,
                    TimeSpan.FromTicks(episode.ResumePositionTicks));
            }
            var restoredPodcastId = previousPodcastItemId ?? _state.Podcasts.CurrentItemId;
        ...
        EnsureQueueOrder(podcasts);
        if (previousPodcastWasPlaying && podcasts.HasCurrentItem)
            podcasts.Play(podcasts.CurrentItem);
        var (wiiM, _) = _sessions.AddOrUpdateTransientSession(
            "wiim",
            "WiiM",
            _wiiMItems,
            output: null,
        ...
        if (wiiM.HasItems)
        {
            var restoredWiiMId = previousWiiMItemId ?? _state.WiiM.SelectedDeviceId;
            var restoredWiiM = wiiM.Items.FirstOrDefault(item =>
                string.Equals(item.Id, restoredWiiMId, StringComparison.OrdinalIgnoreCase));
            if (restoredWiiM is not null) wiiM.SelectItem(restoredWiiM);
        }
        ...
        else if (string.Equals(desiredSessionId, "podcasts", StringComparison.Ordinal))
        {
            _sessions.SelectSession("podcasts");
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(feedAddress)) return null;
        var normalized = NormalizePodcastFeedAddress(feedAddress);
        return _state.Podcasts.Subscriptions.FirstOrDefault(subscription =>
            string.Equals(
                NormalizePodcastFeedAddress(subscription.FeedUrl),
                normalized,
                StringComparison.OrdinalIgnoreCase));
    }

    private bool TryResolvePodcastLibraryActionItems(out IReadOnlyList<MediaItem> items)
    {
        var subscriptions = PodcastMembershipToggle.ResolveSubscriptions(
            _state.Podcasts,
            ActionItems);
        if (subscriptions.Count == 0)
        {
    ...
    private string TogglePodcastDirectoryMembership(
        IReadOnlyList<PodcastSubscriptionSettings> subscriptions,
        SearchResultAction action)
    {
        CapturePodcastState();
        var add = PodcastMembershipToggle.Apply(
            subscriptions,
            action == SearchResultAction.Library
                ? PodcastMembershipCollection.Library
                : PodcastMembershipCollection.Favorites);
        ReloadPodcastSessionItems();
        QueueStateSave(announceFailure: true);
        if (string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal))
            RefreshCurrentView(preferredItemId: subscriptions[0].Id);

        var target = action == SearchResultAction.Library ? "Biblioteki" : "ulubionych";
        var verb = add ? "Dodano do" : "Usunięto z";
        return subscriptions.Count == 1
            ? $"{verb} {target}: {subscriptions[0].Title}"
            : $"{verb} {target}: {FormatItemCount(subscriptions.Count)}";
    }
    ...
    private void PodcastOutput_PlaybackEnded(object? sender, MediaPlaybackEndedEventArgs e)
    {
        ...
        {
            PodcastEpisodeProgress.MarkPlayed(completed);
        }
    ...
    private CommandExecutionResult ExecuteCommand(string commandId)
    {
        ...
        else if (commandId == CommandIds.ToggleLibrary && HasSelectedFolderRow())
        {
            ...
            && string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal)
            && ActionItems.Any(item => item.Kind is MediaItemKind.Podcast or MediaItemKind.Episode))
        {
            if (!TryResolvePodcastLibraryActionItems(out var podcastItems))
            {
                Announce("Nie można odnaleźć podcastu nadrzędnego dla zaznaczonego odcinka");
                return new CommandExecutionResult(true);
            }
            // Odcinek nie ma niezależnego członkostwa w Bibliotece. Skrót na
            // odcinku zawsze dotyczy podcastu nadrzędnego, a więc używa tego
            // samego kanonicznego elementu co skrót na liście podcastów.
            _actionItemsOverride = podcastItems;
        }
        ...
        FolderContentsActionContext? folderContext)
    {
        ...
        if (commandId == CommandIds.AddWiiMNetworkStream)
        {
            AddWiiMNetworkStream();
            return new CommandExecutionResult(true);
        }
        if (commandId == CommandIds.ViewWiiMNetworkStreams)
        {
            ShowWiiMNetworkStreams();
            return new CommandExecutionResult(true);
        }
        if (commandId == CommandIds.ImportWiiMNetworkStreams)
        {
        ...
        if (string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal))
        {
            ...
            {
                var selectedDevice = ActionItem ?? (_sessions.Current.HasCurrentItem ? _sessions.Current.CurrentItem : null);
                if (selectedDevice is null)
                {
                    Announce("Brak wybranego urządzenia WiiM. Ctrl+F5 otwiera wykrywanie i dodawanie urządzeń");
                    return new CommandExecutionResult(false);
                }
                if (IsWiiMNetworkStream(selectedDevice))
                {
                    _ = OpenCurrentItemOnWiiMAsync();
                    return new CommandExecutionResult(true);
                }
            ...
            {
                AnnounceWiiMTime(commandId);
                return new CommandExecutionResult(true);
            }
            if (!CommandAvailableInWiiM(commandId))
            {
                Announce("To polecenie nie jest dostępne w sesji WiiM");
                return new CommandExecutionResult(false);
            }
        }
        if (commandId is CommandIds.SortCollectionByAdded
        ...
        if (commandId == CommandIds.AddPodcast)
        {
        ...
        if (commandId == CommandIds.ViewPodcastInProgress)
        {
            ...
            PrepareViewFocusContext(MediaList.Items.Count == 0
                ? "W trakcie słuchania, brak rozpoczętych odcinków"
                : PodcastInProgressViewName);
            RestoreMediaListFocusAfterRefresh();
            return new CommandExecutionResult(true);
        }
        ...
        if (commandId == CommandIds.RefreshLocalLibrary
            && string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal))
        {
            _ = TryResolveCurrentPodcastSubscription(out _)
                ? RefreshCurrentPodcastAsync()
                : RefreshAllPodcastsAsync();
            return new CommandExecutionResult(true);
        }
        if (commandId == CommandIds.RefreshLocalLibrary
            && !string.Equals(_sessions.Current.Id, "local", StringComparison.Ordinal))
        {
            Announce("F5 nie ma polecenia w bieżącej sesji");
            return new CommandExecutionResult(false);
        }
        ...
        if (commandId == CommandIds.ToggleLibrary
            && string.Equals(changedSession?.Id, "podcasts", StringComparison.Ordinal))
        {
            // RefreshCurrentView reads subscription membership from the
            // persistent podcast model, so synchronize it before rebuilding
            // an episode list.  If its parent has just been removed, return to
            // the list of subscribed podcasts instead of leaving a detached
            // episode view on screen.
            CapturePodcastState();
            if (!_playerViewActive
                && TryGetPodcastIdFromView(_currentView, out var openPodcastId)
                && changedItems.Any(item => item.Kind == MediaItemKind.Podcast
                    && string.Equals(item.Id, openPodcastId, StringComparison.Ordinal)
                    && !item.IsInLibrary))
            {
                _currentView = "Biblioteka";
                var podcastNavigation = GetSessionNavigationState("podcasts");
                podcastNavigation.CurrentView = _currentView;
                podcastNavigation.LastLibraryView = _currentView;
                RestoreFilterForCurrentView(podcastNavigation);
            }
        }
        ...
        if (sessionChanged)
        {
            ...
            else if (string.Equals(sessionBeforeCommand.Id, "podcasts", StringComparison.Ordinal))
            {
                CapturePodcastState();
                QueueStateSave();
            }
        ...
        else if (commandId is CommandIds.PlaybackRateDown
            or CommandIds.PlaybackRateUp
            or CommandIds.PlaybackRateReset
            && result.Handled
            && string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal))
        {
            ...
            var episode = FindPodcastEpisodeSettings(item);
            var subscription = FindPodcastSubscriptionSettings(item);
            if (episode?.PlaybackRateOverride is not null)
            {
                episode.PlaybackRateOverride = _sessions.Current.PlaybackRate;
            }
            else if (subscription?.PlaybackRateOverride is not null)
            {
                subscription.PlaybackRateOverride = _sessions.Current.PlaybackRate;
            }
            else
            {
        ...
        else if (savesPlaybackBoundary && result.Handled)
        {
            ...
            if (string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal)) CapturePodcastState();
            QueueStateSave();
        }
        return result;
    }
    ...
    private void RefreshCurrentView(int? fallbackIndex = null, string? preferredItemId = null)
    {
        ...
        {
            if (tidalContainer.IsArtistOverview)
            {
                _unfilteredItems = CreateTidalArtistSectionRows(tidalContainer.Container);
        ...
        {
            var subscription = _state.Podcasts.Subscriptions.FirstOrDefault(candidate =>
                string.Equals(candidate.Id, podcastId, StringComparison.Ordinal)
        ...
        if (_currentView == "Biblioteka")
        {
            ...
            {
                items = items.Where(IsWiiMNetworkStream);
            }
            if (!string.Equals(_sessions.Current.Id, "local", StringComparison.Ordinal))
    ...
    private bool CurrentViewSupportsCollectionSorting() =>
        ...
        || string.Equals(_sessions.Current.Id, "tidal", StringComparison.Ordinal)
           && (string.Equals(_currentView, "Playlisty", StringComparison.Ordinal)
               || _tidalContainerViews.TryGetValue(_currentView, out var sortableTidalView)
                  && !sortableTidalView.IsArtistOverview)
        || string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal)
           && string.Equals(_currentView, PodcastInboxViewName, StringComparison.Ordinal)
    ...
    private async Task OpenTidalContainerAsync(MediaItem container, ArtistBrowseSection? artistSection = null)
    {
        if (container.ExternalId is not { Length: > 0 }) return;
        if (container.Kind == MediaItemKind.Artist && artistSection is null)
        {
            OpenTidalArtistOverview(container);
            return;
        }
        _tidalNavigationVersion++;
        ...
        var label = artistSection?.Label() ?? TidalContainerLabel(container.Kind);
        try
        {
            ...
            foreach (var item in items)
            {
                ...
                if (container.Kind == MediaItemKind.Artist
                    && item.Kind is MediaItemKind.Album or MediaItemKind.Track
            ...
            var viewName = TidalSectionView(container, artistSection);
            _tidalContainerViews[viewName] = new TidalContainerViewState(container, items, artistSection);
        ...
        IReadOnlyList<MediaItem> items)
    {
        ...
        {
            _tidalContainerViews[viewName] = _tidalContainerViews[viewName] with { Items = items };
        }
    ...
    public void GoToRelatedAlbum()
    {
        if (string.Equals(ActionSession.Id, "tidal", StringComparison.Ordinal))
        {
            ...
            {
                OpenTidalArtistOverview(artist);
                SelectMediaItem(TidalArtistSectionRowId(artist, ArtistBrowseSection.Albums));
                _ = OpenTidalContainerAsync(artist, ArtistBrowseSection.Albums);
                return;
            }
    ...
    private static bool IsTopLevelBrowserView(string viewName) => viewName is
        ...
        or PodcastInboxViewName
        or PodcastInProgressViewName
        or "Pobrane";
    ...
    private void RemoveSelected()
    {
        ...
        if (string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal)
            ...
            && selectedRows.All(row => IsWiiMNetworkStream(row.Item)))
        {
            RemoveWiiMNetworkStreams(selectedRows.Select(row => row.Item).ToArray());
            return;
        }
    ...
    private void OpenSettings(SettingsTarget initialTarget = SettingsTarget.General)
    {
        ...
        ApplyRadioRecognitionMonitoringSetting();
        ApplyEffectiveAudioProcessingForCurrentLocalItem();
    ...
    private void ApplyRadioRecognitionMonitoringSetting()
    {
        _radioRecognitionMonitoring = _state.Radio.AutomaticTrackRecognitionEnabled;
        RefreshRadioRecognitionScheduleAfterSourceChange();
        UpdateFileMenuForCurrentSession();
    ...
    {
        if (string.Equals(ActionSession.Id, "wiim", StringComparison.Ordinal)
            && IsWiiMNetworkStream(ActionItem))
        {
            EditWiiMNetworkStream();
            return;
        }
    ...
    private void EditWiiMNetworkStream()
    {
        var item = ActionItem;
        if (!IsWiiMNetworkStream(item) || string.IsNullOrWhiteSpace(item!.Source))
        {
            Announce("Wybierz jeden zapisany strumień WiiM");
            return;
        }
        var saved = _state.WiiM.NetworkStreams.FirstOrDefault(stream =>
            string.Equals(stream.Id, item.Id, StringComparison.Ordinal));
        if (saved is null)
        {
            Announce("Nie można odnaleźć tego strumienia na zapisanej liście AMC");
            return;
        }

        var dialog = new RadioStationWindow(item.Title, item.Source, wiiMNetworkStream: true)
        {
            Owner = this
        };
        if (dialog.ShowDialog() != true)
        ...
        var duplicate = _state.WiiM.NetworkStreams.FirstOrDefault(stream =>
            !string.Equals(stream.Id, saved.Id, StringComparison.Ordinal)
            && string.Equals(stream.StreamUrl, normalizedUrl, StringComparison.OrdinalIgnoreCase));
        if (duplicate is not null)
        {
            AnnounceEssential($"Ten adres jest już zapisany jako: {duplicate.Name}");
            RestoreItemActionFocus();
            return;
        }

        saved.Name = dialog.StationName;
        saved.StreamUrl = normalizedUrl;
        ShowWiiMNetworkStreams(saved.Id, $"Zapisano strumień WiiM: {saved.Name}");
    }

    private void RemoveWiiMNetworkStreams(IReadOnlyList<MediaItem> items)
    {
    ...
    private string FormatListItem(MediaItem item)
    {
        var homogeneousView = _currentView is "Albumy" or "Playlisty"
            || string.Equals(_sessions.Current.Id, "radio", StringComparison.Ordinal)
               && item.Kind == MediaItemKind.Station;
        var label = string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal)
            && IsWiiMNetworkStream(item)
    ...
    private void RefreshPlaybackIndicators()
    {
        ...
        {
            if (row.Bookmark is not null || row.PlaylistId is not null || row.ArtistSection is not null) continue;
    ...
    public void ShowWiiMDeviceManager()
    {
        ...
        dialog.ShowDialog();
        _wiiMSnapshots.Clear();
        foreach (var (id, snapshot) in dialog.Snapshots) _wiiMSnapshots[id] = snapshot;
        if (dialog.Changed)
        {
    ...
    {
        await RefreshWiiMDevicesAsync(announceResult: true);
    }

    private async Task RefreshWiiMDevicesAsync(bool announceResult)
    {
        if (_wiiMRefreshInProgress) return;
        var devices = _state.WiiM.Devices.ToArray();
        if (devices.Length == 0)
        {
            if (announceResult) Announce("Brak zapisanych urządzeń WiiM. Ctrl+F5 otwiera wykrywanie i dodawanie urządzeń");
            return;
        }
        ...
        try
        {
            ...
            var results = await Task.WhenAll(devices.Select(async device =>
            {
                try
                {
                    ...
                    or TaskCanceledException or IOException or FormatException or ArgumentException)
                {
                    return (Device: device, Snapshot: (WiiMDeviceSnapshot?)null, Error: exception);
                }
            }));
            if (_isClosing) return;
            var available = 0;
            foreach (var result in results)
            {
                ...
                _wiiMSnapshots[result.Device.Id] = result.Snapshot;
                available++;
            }
            RefreshWiiMSessionItems(selectedId);
            QueueStateSave(announceFailure: true);
            if (announceResult)
            {
                Announce(available == devices.Length
                    ? devices.Length == 1
                        ? "Odświeżono urządzenie WiiM"
                        : $"Odświeżono urządzenia WiiM: {available}"
                    : $"Dostępne urządzenia WiiM: {available} z {devices.Length}");
            }
        }
        catch (OperationCanceledException) when (_wiiMCancellation.IsCancellationRequested)
        {
    ...
    private async Task ActivateWiiMDeviceAsync(MediaItem item)
    {
        var session = _sessions.FindSession("wiim");
        if (session is null || !session.SelectItem(item))
        {
            Announce("Nie można wybrać urządzenia WiiM");
            return;
        }
        _state.WiiM.SelectedDeviceId = item.Id;
        var gateEntered = false;
        WiiMDeviceSnapshot? snapshot;
        try
        {
            await _wiiMDeviceOperationGate.WaitAsync(_wiiMCancellation.Token);
            gateEntered = true;
            snapshot = await RefreshSelectedWiiMDeviceAsync(item, announceFailure: true);
        }
        catch (OperationCanceledException) when (_wiiMCancellation.IsCancellationRequested)
        {
            return;
        ...
        if (snapshot is null || _isClosing) return;
        QueueStateSave(announceFailure: true);
        _nextWiiMPlayerRefreshUtc = DateTime.UtcNow.AddSeconds(3);
        _playerFocusContextPrefix = $"Wybrano urządzenie {snapshot.Device.Name}";
        ShowPlayerView();
    }

    private async Task<WiiMDeviceSnapshot?> RefreshSelectedWiiMDeviceAsync(
        MediaItem item,
        bool announceFailure)
    {
        ...
        if (device is null)
        {
            if (announceFailure) Announce("Nie można odnaleźć zapisanego urządzenia WiiM");
            return null;
        }
        try
        {
        ...
        int? requestedVolume = null)
    {
        ...
        var session = _sessions.Current;
        if (!session.HasCurrentItem
            || !TryGetWiiMDevice(session.CurrentItem.Id, out var device))
        {
        ...
        try
        {
            ...
            TimeSpan? requestedPosition = null;
            if (commandId is CommandIds.SeekToTime or CommandIds.SeekToPercentage)
            {
                if (snapshot.Playback.Duration <= TimeSpan.Zero)
                {
                    Announce("Przewijanie jest niedostępne, ponieważ urządzenie nie podało czasu trwania");
                    return;
                }
                var mode = commandId == CommandIds.SeekToTime
                    ? SeekInputMode.Time
                    : SeekInputMode.Percentage;
                var dialog = new SeekPositionWindow(mode, snapshot.Playback.Duration) { Owner = this };
                if (dialog.ShowDialog() != true)
                {
                    FocusPlayerView();
                    return;
                }
                requestedPosition = mode == SeekInputMode.Time
                    ? dialog.Position
                    : TimeSpan.FromTicks((long)Math.Round(
                        snapshot.Playback.Duration.Ticks * (dialog.Percentage / 100d)));
            }
            else if (CommandIds.TryParseSeekPercent(commandId, out var percentage))
            {
                if (snapshot.Playback.Duration <= TimeSpan.Zero)
                {
                    Announce("Skok procentowy jest niedostępny, ponieważ urządzenie nie podało czasu trwania");
                    return;
                }
                requestedPosition = TimeSpan.FromTicks((long)Math.Round(
                    snapshot.Playback.Duration.Ticks * (percentage / 100d)));
            }

            switch (commandId)
            {
                case CommandIds.PlayPause:
                    await _wiiMClient.TogglePlayPauseAsync(device.Address, _wiiMCancellation.Token);
                    break;
                case CommandIds.Previous:
                    await _wiiMClient.PreviousAsync(device.Address, _wiiMCancellation.Token);
                    break;
                ...
                case CommandIds.ToggleMuteCurrentSession:
                    ...
                    break;
                case CommandIds.SeekBackward10:
                    requestedPosition = snapshot.Playback.Position - TimeSpan.FromSeconds(10);
                    break;
                case CommandIds.SeekForward10:
                    requestedPosition = snapshot.Playback.Position + TimeSpan.FromSeconds(10);
                    break;
                case CommandIds.SeekBackward30:
                    requestedPosition = snapshot.Playback.Position - TimeSpan.FromSeconds(30);
                    break;
                case CommandIds.SeekForward30:
                    requestedPosition = snapshot.Playback.Position + TimeSpan.FromSeconds(30);
                    break;
                case CommandIds.SeekBackward60:
                    requestedPosition = snapshot.Playback.Position - TimeSpan.FromSeconds(60);
                    break;
                case CommandIds.SeekForward60:
                    requestedPosition = snapshot.Playback.Position + TimeSpan.FromSeconds(60);
                    break;
                ...
                case CommandIds.TrackEnd:
                    if (snapshot.Playback.Duration <= TimeSpan.Zero)
                    {
                        Announce("Koniec materiału jest nieznany");
                        return;
                    }
                    requestedPosition = snapshot.Playback.Duration;
                    break;
            }
            if (requestedPosition is { } position)
            {
                var maximum = snapshot.Playback.Duration > TimeSpan.Zero
                    ? snapshot.Playback.Duration
                    : TimeSpan.MaxValue;
                position = position < TimeSpan.Zero ? TimeSpan.Zero : position > maximum ? maximum : position;
    ...
    private async Task ShowWiiMDevicePresetsAsync()
    {
        var item = ActionItem ?? (_sessions.Current.HasCurrentItem ? _sessions.Current.CurrentItem : null);
        ...
        try
        {
            ...
            var refreshed = await _wiiMClient.ReadSnapshotAsync(device.Address, _wiiMCancellation.Token);
            ApplyWiiMSnapshot(device, refreshed);
            _playerFocusContextPrefix = $"Uruchomiono preset {number}, {refreshed.Track.Title}".TrimEnd(' ', ',');
            ShowPlayerView();
        }
        catch (OperationCanceledException) when (_wiiMCancellation.IsCancellationRequested)
        {
    ...
    private async Task PollActiveWiiMDeviceAsync()
    {
        if (!_playerViewActive
            || !string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal)
            || !_sessions.Current.HasCurrentItem
            || !TryGetWiiMDevice(_sessions.Current.CurrentItem.Id, out var device)
            || !await _wiiMDeviceOperationGate.WaitAsync(0))
        {
            return;
        }
        try
        ...
        {
            DiagnosticLog.Warning("wiim-poll", $"Okresowe odświeżenie {device.Address} nie powiodło się: {exception.GetType().Name}.");
            _nextWiiMPlayerRefreshUtc = DateTime.UtcNow.AddSeconds(15);
        }
        finally
        {
            _wiiMDeviceOperationGate.Release();
        }
    }
    ...
    {
        device = _state.WiiM.Devices.FirstOrDefault(candidate =>
    ...
    private void AnnounceWiiMTime(string commandId)
    {
        if (!_sessions.Current.HasCurrentItem
            || !_wiiMSnapshots.TryGetValue(_sessions.Current.CurrentItem.Id, out var snapshot))
        {
            Announce("Stan czasu urządzenia nie jest jeszcze dostępny");
            return;
        }
        var playback = snapshot.Playback;
        if (commandId == CommandIds.TimeElapsed)
            Announce($"Czas od początku: {CommandRouter.FormatTime(playback.Position)}");
        else if (commandId == CommandIds.TimeRemaining)
            Announce(playback.Duration <= TimeSpan.Zero
                ? "Czas pozostały jest nieznany"
            ...
            Announce(playback.Duration <= TimeSpan.Zero
                ? "Czas całkowity jest nieznany"
                : $"Czas całkowity: {CommandRouter.FormatTime(playback.Duration)}");
    }

    private void AnnounceWiiMCommandResult(
        string commandId,
        WiiMDeviceSnapshot snapshot,
        TimeSpan? requestedPosition)
    {
        if (commandId is CommandIds.VolumeUp5 or CommandIds.VolumeDown5
            ...
            or CommandIds.ToggleMuteCurrentSession)
        {
            if (_state.Settings.Messages.VolumeMessages)
                Announce(snapshot.Playback.Muted ? "Wyciszono" : $"Głośność {snapshot.Playback.Volume}%");
            return;
        }
        if (requestedPosition is not null)
        {
            if (_state.Settings.Messages.SeekMessages)
                Announce(CommandRouter.FormatTime(snapshot.Playback.Position));
            return;
        }
        if (!_state.Settings.Messages.PlaybackMessages) return;
        if (commandId == CommandIds.PlayPause)
            Announce(snapshot.Playback.State);
        else if (commandId is CommandIds.Previous or CommandIds.Next)
            Announce(string.IsNullOrWhiteSpace(snapshot.Track.Title)
                ? snapshot.Playback.State
                : snapshot.Track.Title);
    }

    private static bool IsWiiMTransportCommand(string commandId) => commandId is
    ...
    private static void UpdateWiiMRegistration(
        WiiMDeviceSettings device,
        WiiMDeviceSnapshot snapshot)
    {
        device.Address = snapshot.Device.Address;
        device.DisplayName = snapshot.Device.Name;
        device.Model = snapshot.Device.Model;
        device.Firmware = snapshot.Device.Firmware;
        device.LastSeenUtcTicks = DateTime.UtcNow.Ticks;
    }

    private void RefreshWiiMSessionItems(
        string? preferredItemId = null,
        bool updateAccessiblePlayerName = false)
    {
        preferredItemId ??= string.Equals(_sessions.Current.Id, "wiim", StringComparison.OrdinalIgnoreCase)
            ? SelectedItem?.Id
            : null;
        LoadPersistedWiiM();
        var session = _sessions.FindSession("wiim");
        session?.ReplaceItems(_wiiMItems);
        if (session is not null && preferredItemId is not null)
        {
            var preferredItem = session.Items.FirstOrDefault(item =>
                string.Equals(item.Id, preferredItemId, StringComparison.OrdinalIgnoreCase));
            if (preferredItem is not null) session.SelectItem(preferredItem);
        }
        if (!string.Equals(_sessions.Current.Id, "wiim", StringComparison.OrdinalIgnoreCase)) return;
        if (_playerViewActive)
        {
            UpdatePlayerView(updateAccessiblePlayerName);
            UpdatePlaybackStatusBar();
            UpdateWindowTitle();
            return;
        }
    ...
    private static string PodcastContentsView(string podcastId) =>
        $"{PodcastContentsViewPrefix}{podcastId}";

    private static bool TryGetPodcastIdFromView(string viewName, out string podcastId)
    {
        if (viewName.StartsWith(PodcastContentsViewPrefix, StringComparison.Ordinal)
            && viewName.Length > PodcastContentsViewPrefix.Length)
        {
            podcastId = viewName[PodcastContentsViewPrefix.Length..];
            return true;
        }
        podcastId = string.Empty;
        return false;
    }
    ...
    private void Window_PreviewKeyDown(object sender, KeyEventArgs e)
    {
        ...
        if (Keyboard.Modifiers == ModifierKeys.Control
            && e.Key == Key.E
            && _sessions.Current.Id is "radio" or "wiim")
        {
            ExecuteCommand(string.Equals(_sessions.Current.Id, "radio", StringComparison.Ordinal)
                ? CommandIds.ExportRadioFavorites
                : CommandIds.ExportWiiMNetworkStreams);
            e.Handled = true;
            return;
        }

        var openDataCommand = MainWindowShortcutRouter.ResolveOpenData(
            e.Key,
            Keyboard.Modifiers,
            _sessions.Current.Id);
        if (openDataCommand is not null)
        {
            ExecuteCommand(openDataCommand);
            e.Handled = true;
            return;
        }
    ...
    private bool TryResolveKeyboardHelpCommand(Key key, ModifierKeys modifiers, out string commandId)
    {
        ...
        commandId = (modifiers, key) switch
        {
            ...
            (ModifierKeys.Control, Key.L)
                when !string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal) =>
    ...
    private bool TryHandleLocalNavigationShortcut(KeyEventArgs e)
    {
        ...
        if (Keyboard.Modifiers == ModifierKeys.Control
            ...
            && string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal))
        {
            Announce("WiiM nie udostępnia Biblioteki. Ctrl+O otwiera zapisane strumienie");
            return true;
        }
        var commandId = (Keyboard.Modifiers, key) switch
        {
            ...
            (ModifierKeys.Control | ModifierKeys.Shift, Key.I)
                when string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal) =>
                    CommandIds.ViewPodcastInProgress,
            (ModifierKeys.Control | ModifierKeys.Shift, Key.B) => CommandIds.AddNamedBookmark,
            (ModifierKeys.Control, Key.K) => CommandIds.FilterCurrent,
            (ModifierKeys.Control, Key.F) => CommandIds.SearchCurrent,
            (ModifierKeys.Control | ModifierKeys.Shift, Key.A) => CommandIds.ViewAlbums,
    ...
    private bool TryHandleItemActionShortcut(KeyEventArgs e)
    {
        ...
        if (commandId is null) return false;
        if (!_playerViewActive
            && SelectedItem?.Kind == MediaItemKind.Podcast
            && commandId is CommandIds.AddQueue
                or CommandIds.TogglePlayNext
                or CommandIds.ManagePlaylists)
        {
            Announce("To polecenie dotyczy odcinków. Otwórz podcast Enterem");
            return true;
        }
        ExecuteCommand(commandId);
        return true;
        ...
        bool allServices)
    {
        ...
        {
            var mappedSubscriptions = results
                .Select(result => FindPodcastSubscriptionByFeed(result.Item.Source))
                .ToArray();
            if (mappedSubscriptions.All(subscription => subscription is not null))
            {
                return TogglePodcastDirectoryMembership(
                    mappedSubscriptions
                        .Select(subscription => subscription!)
                        .DistinctBy(subscription => subscription.Id, StringComparer.Ordinal)
                        .ToArray(),
                    action);
            }
    ...
    {
        var key = e.Key == Key.System ? e.SystemKey : e.Key;
        if (SelectedArtistSection is { } artistSection && Keyboard.Modifiers == ModifierKeys.None
            && key is Key.Left or Key.Right)
        {
            Announce($"{artistSection.Label()}. Naciśnij Enter, aby otworzyć");
            e.Handled = true;
            return;
        }
    ...
    private void OpenOnWiiM_Click(object sender, RoutedEventArgs e) => ExecuteCommand(CommandIds.OpenOnWiiM);
    ...
    private void MediaContextMenu_Opened(object sender, RoutedEventArgs e)
    {
        if (SelectedArtistSection is { } artistSection)
        {
            if (sender is ContextMenu categoryMenu) categoryMenu.IsOpen = false;
            Announce($"{artistSection.Label()}. Naciśnij Enter, aby otworzyć");
            return;
        }
        if ((MediaList.SelectedItem as MediaItemRow)?.LoadMorePodcastViewName is not null)
        ...
        var openOnWiiMAvailable = !wiiMSession
            && items.Count == 1
            && TryGetWiiMPlayableUri(actionItem, out _);
        OpenOnWiiMMenuItem.Visibility = openOnWiiMAvailable
            ? Visibility.Visible
            : Visibility.Collapsed;
        if (openOnWiiMAvailable)
        {
            MenuAccessibility.SetPresentation(
                OpenOnWiiMMenuItem,
                ResolveWiiMPlaybackTarget() is { } playbackTarget
                    ? $"Otwórz w {playbackTarget.DisplayName}"
                    : "Otwórz w WiiM");
        }
        ...
        var wiiMNetworkStream = wiiMSession && IsWiiMNetworkStream(actionItem);
        ...
        SetContextMenuItemPresentation(
            PlaybackMenuItem,
            playbackLabel,
            localAlbumContainer || playlistContainer || folderNavigationRow || podcastHeader || wiiMNetworkStream
                ? "Enter"
                : "Ctrl+Enter");
        ...
        QueueMenuItem.Visibility = radioSession
            ...
            ? Visibility.Collapsed
            : Visibility.Visible;
        FavoriteMenuItem.Visibility = wiiMSession
            || localAlbumContainer
            || (playlistContainer && membershipItems.Count == 0)
            || (folderNavigationRow && membershipItems.Count == 0)
        ...
        ItemPlaybackOptionsMenuItem.Visibility = wiiMSession
            || playlistContainer
            ...
            ? Visibility.Collapsed
            : Visibility.Visible;
        if (ItemPlaybackOptionsMenuItem.Visibility == Visibility.Visible)
        {
            SetContextMenuItemPresentation(
                ItemPlaybackOptionsMenuItem,
                podcastHeader
                    ? "Zmień opcje podcastu"
                    : podcastEpisode
                        ? "Zmień opcje odcinka podcastu"
        ...
        var podcastRenameItem = SelectedBookmark is null
            ...
            && string.Equals(_sessions.Current.Id, "podcasts", StringComparison.Ordinal);
        var wiiMStreamRenameItem = SelectedBookmark is null
            && items.Count == 1
            && wiiMNetworkStream;
        RenameLibraryItemMenuItem.Visibility = localRenameItem || radioRenameItem || podcastRenameItem || wiiMStreamRenameItem
            ? Visibility.Visible
            : Visibility.Collapsed;
        if (wiiMStreamRenameItem)
            SetContextMenuItemPresentation(RenameLibraryItemMenuItem, "Edytuj nazwę i adres strumienia WiiM", "F2");
        else if (radioRenameItem)
        ...
        if (wiiMSession)
        {
            SetContextMenuItemPresentation(PlaybackMenuItem, "Wybierz urządzenie i otwórz sterowanie", "Enter");
            SetContextMenuItemPresentation(InformationMenuItem, "Właściwości i stan urządzenia", "Alt+Enter");
            SetContextMenuItemPresentation(CopyNameMenuItem, "Kopiuj nazwę urządzenia", "Ctrl+C");
            SetContextMenuItemPresentation(CopyLocationMenuItem, "Kopiuj adres IP urządzenia", "Ctrl+Shift+C");
            PlayNextMenuItem.Visibility = Visibility.Collapsed;
            QueueMenuItem.Visibility = Visibility.Collapsed;
    ...
    private void PlayerContextMenu_Opened(object sender, RoutedEventArgs e)
    {
        var item = _sessions.Current.CurrentItem;
        var wiiMSession = string.Equals(_sessions.Current.Id, "wiim", StringComparison.Ordinal);
        var playerOpenOnWiiMAvailable = !wiiMSession && TryGetWiiMPlayableUri(item, out _);
        PlayerOpenOnWiiMMenuItem.Visibility = playerOpenOnWiiMAvailable
            ? Visibility.Visible
            : Visibility.Collapsed;
        if (playerOpenOnWiiMAvailable)
        {
            MenuAccessibility.SetPresentation(
                PlayerOpenOnWiiMMenuItem,
                ResolveWiiMPlaybackTarget() is { } playbackTarget
                    ? $"Otwórz w {playbackTarget.DisplayName}"
                    : "Otwórz w WiiM");
        }
        ...
        SetContextMenuItemPresentation(
            ...
            wiiMSession
                && _wiiMSnapshots.TryGetValue(item.Id, out var wiiMSnapshot)
                && string.Equals(wiiMSnapshot.Playback.State, "odtwarzanie", StringComparison.OrdinalIgnoreCase)
                    ? "Wstrzymaj"
        ...
        if (wiiMSession)
        {
            ...
            PlayerInterTrackSilenceMenuItem.Visibility = Visibility.Collapsed;
            PlayerItemPlaybackOptionsMenuItem.Visibility = Visibility.Collapsed;
            PlayerAddBookmarkMenuItem.Visibility = Visibility.Collapsed;
            PlayerAddNamedBookmarkMenuItem.Visibility = Visibility.Collapsed;
            PlayerBookmarksMenuItem.Visibility = Visibility.Collapsed;
            PlayerBeforeChaptersSeparator.Visibility = Visibility.Collapsed;
            PlayerChaptersMenuItem.Visibility = Visibility.Collapsed;
            PlayerAddNamedChapterMenuItem.Visibility = Visibility.Collapsed;
            PlayerPreviousChapterMenuItem.Visibility = Visibility.Collapsed;
            PlayerNextChapterMenuItem.Visibility = Visibility.Collapsed;
            PlayerBeforeClipSeparator.Visibility = Visibility.Collapsed;
            PlayerMarkClipStartMenuItem.Visibility = Visibility.Collapsed;
            PlayerMarkClipEndMenuItem.Visibility = Visibility.Collapsed;
            PlayerJumpClipStartMenuItem.Visibility = Visibility.Collapsed;
            PlayerJumpClipEndMenuItem.Visibility = Visibility.Collapsed;
            PlayerPreviousClipBoundaryMenuItem.Visibility = Visibility.Collapsed;
            PlayerNextClipBoundaryMenuItem.Visibility = Visibility.Collapsed;
            PlayerExportClipMenuItem.Visibility = Visibility.Collapsed;
            PlayerRemoveClipMenuItem.Visibility = Visibility.Collapsed;
            PlayerClearClipMenuItem.Visibility = Visibility.Collapsed;
            PlayerPlayNextMenuItem.Visibility = Visibility.Collapsed;
            PlayerQueueMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioRecordingMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioPauseRecordingMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioSplitRecordingMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioAddScheduleMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioSchedulesMenuItem.Visibility = Visibility.Collapsed;
            PlayerRecognizeRadioTrackMenuItem.Visibility = Visibility.Collapsed;
            PlayerMonitorRadioRecognitionMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioRecognitionAnnouncementsMenuItem.Visibility = Visibility.Collapsed;
            PlayerRadioRecognitionHistoryMenuItem.Visibility = Visibility.Collapsed;
            ...
            PlayerGoToAlbumMenuItem.Visibility = Visibility.Collapsed;
            PlayerGoToArtistMenuItem.Visibility = Visibility.Collapsed;
            PlayerCopyNameMenuItem.Visibility = Visibility.Visible;
            PlayerCopyLocationMenuItem.Visibility = Visibility.Visible;
            SetContextMenuItemPresentation(PlayerCopyNameMenuItem, "Kopiuj nazwę urządzenia", "Ctrl+C");
            SetContextMenuItemPresentation(PlayerCopyLocationMenuItem, "Kopiuj adres IP urządzenia", "Ctrl+Shift+C");
            PlayerShowLocalFileInFolderMenuItem.Visibility = Visibility.Collapsed;
            PlayerOfficialApplicationMenuItem.Visibility = Visibility.Collapsed;
            PlayerRemoveLocalItemMenuItem.Visibility = Visibility.Collapsed;
            return;
        }
        ...
        var podcastPlaybackOptions = string.Equals(
                ...
                StringComparison.Ordinal)
            && item.Kind == MediaItemKind.Episode;
        PlayerItemPlaybackOptionsMenuItem.Visibility = localPlaybackOptions || podcastPlaybackOptions
            ? Visibility.Visible
            : Visibility.Collapsed;
        ...
        else if (podcastPlaybackOptions)
        {
            SetContextMenuItemPresentation(
                PlayerItemPlaybackOptionsMenuItem,
                "Zmień opcje bieżącego odcinka podcastu",
    ...
    private static string GetShareableLocation(MediaItem item, string sessionId)
    {
        if (string.Equals(sessionId, "podcasts", StringComparison.Ordinal)
            && Uri.TryCreate(item.Source, UriKind.Absolute, out var podcastSource)
            && podcastSource.Scheme is "http" or "https")
        {
            return podcastSource.AbsoluteUri;
        }
    ...
    private void PodcastInProgressView_Click(object sender, RoutedEventArgs e) => ExecuteCommand(CommandIds.ViewPodcastInProgress);
