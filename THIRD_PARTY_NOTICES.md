...
- **yt-dlp** — opcjonalny, zarządzany program uruchamiany poza procesem AMC wyłącznie do rozwiązania jawnie wskazanego publicznego adresu YouTube. AMC pobiera oficjalny plik `yt-dlp.exe` oraz `SHA2-256SUMS` z [wydań projektu yt-dlp](https://github.com/yt-dlp/yt-dlp/releases), weryfikuje sumę i wersję przed uaktywnieniem i nie odczytuje cookies ani profilu przeglądarki. Główny projekt jest udostępniany na warunkach The Unlicense; samodzielny plik wykonywalny zawiera także zależności i informacje licencyjne opisane przez jego dostawcę.
- **TIDAL SDK for Web, moduł Player 0.20.1** — oficjalny moduł TIDAL używany wyłącznie do zgodnego odtwarzania chronionych materiałów katalogu. AMC przekazuje mu identyfikator utworu i krótkotrwały token użytkownika; nie wydobywa ani nie ujawnia adresów chronionego dźwięku. Projekt TIDAL SDK for Web jest udostępniany na licencji Apache 2.0: https://github.com/tidal-music/tidal-sdk-web. Pełny tekst licencji znajduje się w `licenses/TIDAL-SDK-Web-Apache-2.0.txt`.
- **Microsoft Edge WebView2 SDK 1.0.4191.47** — host oficjalnego modułu TIDAL Player w systemie Windows. Do działania potrzebny jest środowiskowy składnik Microsoft Edge WebView2 Runtime; pakiet SDK jest dystrybuowany przez Microsoft na warunkach dołączonych do oficjalnego pakietu NuGet.
