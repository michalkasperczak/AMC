...
Od `alpha.33` lokalna sesja korzysta z rzeczywistego wyjścia dźwięku Windows. Enter i `Ctrl+Enter` uruchamiają lub wstrzymują wybrany plik, a Spacja, przewijanie, głośność i polecenia czasu sterują tym samym torem. `Alpha.34` wprowadziła lokalne przypisania czasu, lecz test NVDA wykazał, że nie docierały niezawodnie do aplikacji. W `alpha.35` `Ctrl+E`, `Ctrl+R` i `Ctrl+T` są przechwytywane na granicy komunikatów okna, a transport strzałkami działa bez prefiksu z fokusem na głównej liście. Implementacja używa kodeków dostępnych w Windows i wyjścia współdzielonego, nie instaluje globalnych pakietów kodeków i nie wycisza czytnika ekranu. Lista lokalna jest na razie przechowywana tylko do zamknięcia programu.
...
- TIDAL, Apple Music i WiiM są obecnie sesjami demonstracyjnymi. Sesja lokalna odtwarza prawdziwe pliki, ale nie jest jeszcze trwałą biblioteką.
