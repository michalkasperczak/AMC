...
Od `alpha.33` lokalna sesja korzysta z rzeczywistego wyjścia dźwięku Windows. Enter i `Ctrl+Enter` uruchamiają lub wstrzymują wybrany plik, a Spacja, przewijanie, głośność i polecenia czasu sterują tym samym torem. W `alpha.34` informacje o czasie działają w głównym oknie także bez prefiksu przez `Ctrl+E`, `Ctrl+R` i `Ctrl+T`. Implementacja używa kodeków dostępnych w Windows i wyjścia współdzielonego, nie instaluje globalnych pakietów kodeków i nie wycisza czytnika ekranu. Lista lokalna jest na razie przechowywana tylko do zamknięcia programu.
