...
`Ctrl+O` albo menu **Plik → Otwórz pliki audio** otwiera jeden lub wiele lokalnych plików, lecz nie uruchamia ich automatycznie. Pliki otrzymują tymczasową sesję **Lokalne multimedia**, domyślnie w pierwszym wolnym miejscu od 4.
...
Od `alpha.33` lokalna sesja korzysta z rzeczywistego wyjścia dźwięku Windows. Enter i `Ctrl+Enter` uruchamiają lub wstrzymują wybrany plik, a Spacja, przewijanie, głośność i polecenia czasu sterują tym samym torem. Implementacja używa kodeków dostępnych w Windows i wyjścia współdzielonego, nie instaluje globalnych pakietów kodeków i nie wycisza czytnika ekranu. Lista lokalna jest na razie przechowywana tylko do zamknięcia programu.
