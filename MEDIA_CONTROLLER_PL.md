...
Bitrate radia ma trzy wiarygodne źródła: metadane katalogu, parametr raportowany przez faktycznie otwarty dekoder oraz ograniczoną próbę wykonaną na jawne żądanie lewej strzałki. Wykryte `kb/s` i częstotliwość próbkowania są zapisywane przy stacji i pojawiają się w krótkiej informacji oraz pasku. Dla HLS bez deklarowanego pasma albo serwera niepodającego parametrów wartość pozostaje nieznana; bitrate wyjściowego PCM lub MP3 nagrania nie może udawać bitrate źródła. Pasek WinForms ma stałą, krótką nazwę roli, a dynamiczną treść tylko w etykiecie, co pozwala `NVDA+End` odnaleźć pasek bez wielokrotnego czytania tej samej treści.
...
AMC rozróżnia **lokalne presety Radia internetowego** od **presetów urządzenia** WiiM, BluOS lub przyszłego adaptera sprzętowego. Zwykłe Radio ma dwanaście własnych miejsc `1–0`, `-` i `=` przechowywanych przez AMC. Robocza mapa przewiduje `Ctrl+Shift+numer` jako natychmiastowe uruchomienie zajętego miejsca, `Ctrl+P` jako dostępną listę wszystkich miejsc oraz `Ctrl+Shift+P`, a następnie pojedynczy numer, jako krótki tryb przypisania aktualnej lub zaznaczonej stacji. Puste miejsce bezpośrednie niczego nie uruchamia i podaje sposób zapisu. Zastąpienie zajętego miejsca wymaga ponownego naciśnięcia numeru w trybie przypisania albo jawnego potwierdzenia. `Escape` anuluje tryb. Dzięki temu zwykłe wywołanie presetu pozostaje natychmiastowe, a zapis nie uruchamia najpierw poprzedniej stacji, jak stałoby się przy zwykłym podwójnym stuknięciu. Długie przytrzymanie nie jest używane, ponieważ powtarzanie klawisza, układ klawiatury i współpraca z czytnikiem ekranu czynią je mniej przewidywalnym.
...
Sesja **Radio internetowe** ma własną lokalną Bibliotekę i własne Ulubione. Wynik katalogu nie staje się Ulubionym samoczynnie. `Insert` w Bibliotece dodaje ręczny adres, natomiast `F2` zawsze edytuje osobno użytkową nazwę i adres strumienia; zmiana nazwy nie modyfikuje URL, a zmiana URL nie kasuje nazwy. `Alt+Enter` podaje nazwę, rodzaj, sesję, kraj, język, tagi, kodek, bitrate, adres strumienia i stronę stacji, jeśli katalog je udostępnił. Wewnętrzny identyfikator katalogu nie jest czytany przez NVDA.
