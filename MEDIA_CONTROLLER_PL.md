...
Nagrywanie radia jest świadomie uruchamianą funkcją lokalną do prywatnego użytku: `Ctrl+Alt+R` zapisuje od chwili włączenia dekodowany strumień do WAV, domyślnie w `Muzyka\AMC — Nagrania radia`. Nie uruchamia się automatycznie, nie dotyczy TIDAL, Spotify ani Apple Music, nie obchodzi DRM i pozostawia użytkownikowi odpowiedzialność za zgodność z prawem właściwym dla miejsca użycia. Pierwsza wersja bezpiecznie kończy zerwane połączenie; automatyczne ponawianie ze stanem i limitami pozostaje następnym etapem utwardzania.
