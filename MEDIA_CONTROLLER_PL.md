...
AMC rozróżnia **lokalne presety Radia internetowego** od **presetów urządzenia** WiiM, BluOS lub przyszłego adaptera sprzętowego. Zwykłe Radio ma dwanaście własnych miejsc `1–0`, `-` i `=` przechowywanych przez AMC. `Ctrl+Shift+1–0/-/=` natychmiast uruchamia zajęte miejsce, a puste niczego nie zmienia i wyjaśnia sposób przypisania. `Ctrl+Alt+P` otwiera dostępną listę wyłącznie do uruchamiania: Enter i Spacja włączają zajęty preset i nigdy go nie zapisują ani nie nadpisują. Lista pozwala zaznaczyć kilka miejsc Shiftem; `Ctrl+C` kopiuje nazwy zajętych stacji, a `Ctrl+Shift+C` pary nazwa–adres, z pominięciem pustych miejsc. `Ctrl+Alt+Shift+P` otwiera tryb przypisania aktualnej lub zaznaczonej stacji, podaje pierwsze wolne miejsce, przyjmuje pojedynczy klawisz miejsca, a Enter zapisuje. Zajęte miejsce wymaga ponownego wskazania tego samego klawisza i dopiero Entera; wybór innego miejsca zmienia cel, a Escape anuluje całą operację. Delete przygotowuje usunięcie wybranego przypisania, które również wymaga Entera. Dzięki temu zwykłe wywołanie presetu pozostaje natychmiastowe, a zapis nie uruchamia najpierw poprzedniej stacji. Długie przytrzymanie nie jest używane, ponieważ powtarzanie klawisza, układ klawiatury i współpraca z czytnikiem ekranu czynią je mniej przewidywalnym.
...
W bieżącej implementacji **Plików lokalnych** preset może wskazywać plik albo dowolny widoczny folder z zarejestrowanego źródła Biblioteki. Plik jest identyfikowany trwałym identyfikatorem katalogu AMC i po wywołaniu od razu się odtwarza; folder zachowuje pełną ścieżkę, przełącza Bibliotekę na widok Foldery i otwiera zapisany poziom bez uruchamiania dźwięku. Ta sama lista i przypisanie działają w Folderach, Wszystkich plikach, Kolejności własnej, Ulubionych i odtwarzaczu. Preset nie przełącza sam sesji. Gdy plik zniknie z aktywnej Biblioteki albo folder zostanie odłączony, przypisanie pozostaje zapisane i jest oznajmiane jako niedostępne, aby czasowa niedostępność chmury nie usuwała danych użytkownika.

W bieżącej implementacji Radia preset wskazuje jedną stację. Rozszerzenie celu na playlistę jest zaplanowane dopiero po testach playlist. Wywołanie takiego przyszłego presetu ma otworzyć playlistę jako kontekst odtwarzania i uruchomić zapamiętaną w niej stację, a przy jej braku pierwszą. `Page Up` i `Page Down` będą wówczas przechodzić wyłącznie po stacjach tej playlisty. Playlista radia nie jest folderem odtwarzanym automatycznie do końca: transmisja na żywo nie ma naturalnego końca, więc zmiana stacji następuje wyłącznie na polecenie użytkownika.
