...
Nagrywanie radia jest świadomie uruchamianą funkcją lokalną do prywatnego użytku: `Ctrl+Alt+R` zapisuje od chwili włączenia dekodowany strumień, domyślnie w `Muzyka\AMC — Nagrania radia`. Od `alpha.116` obowiązkowym i domyślnym wynikiem jest MP3 kodowany przez systemowy Windows Media Foundation, z jakością docelową 192 kb/s; dla nietypowej częstotliwości próbkowania koder wybiera najbliższy dostępny bitrate. Nie jest to kopiowanie oryginalnych segmentów transmisji: MP3, AAC, OGG i inne obsługiwane źródła przechodzą przez jeden dekodowany tor PCM, dzięki czemu zerwanie HLS lub zmiana kodeka nie pozostawia niespójnego kontenera. Zapis odbywa się do pliku roboczego `.amc-partial`, a docelowa nazwa `.mp3` powstaje atomowo dopiero po zamknięciu kodera. Zatrzymanie, zmiana stacji i kontrolowane zamknięcie aplikacji finalizują plik; awaria kodera usuwa niedokończone dane, jest oznajmiana bez identyfikatora technicznego i nie zatrzymuje radia. Funkcja nie uruchamia się automatycznie, nie dotyczy TIDAL, Spotify ani Apple Music, nie obchodzi DRM i pozostawia użytkownikowi odpowiedzialność za zgodność z prawem właściwym dla miejsca użycia. Automatyczne ponawianie połączenia ze stanem i limitami pozostaje następnym etapem utwardzania.
