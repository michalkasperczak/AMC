...
| `Ctrl+Shift+C` | kopiuj pełną ścieżkę lokalną albo parę nazwa i publiczne łącze do każdego zaznaczonego elementu w usłudze |
...
| `R` w odtwarzaczu Radia | rozpocznij albo zakończ ręczne nagrywanie bieżącej stacji |
| `T` w odtwarzaczu Radia lub widoku Nagrywane | zakończ bieżącą część aktywnego nagrania i rozpocznij nowy plik |
| `Shift+R` w Radiu | otwórz nagrywanie czasowe i harmonogram z opcjonalnym podziałem plików |
...
Stan `alpha.106`: OGG/Vorbis ma zerowaną logiczną oś czasu także wtedy, gdy nagranie jest fragmentem ciągłej transmisji i pierwsza strona dźwięku zachowuje wysoki absolutny numer próbki. Czytnik nie przepisuje ani nie konwertuje źródła. Wyłącza obcinanie według błędnej absolutnej osi NVorbis, wykrywa początek fragmentu podczas krótkiego odczytu nagłówka, odejmuje go od pozycji i końca oraz nakłada bezpieczną granicę na ewidentnie nieprawidłowy czas. Zapobiega to niekończącemu się odczytowi i pełnemu obciążeniu rdzenia po dojściu do końca pliku. Test regresji generuje mały poprawny OGG z przesuniętymi pozycjami stron, sprawdza czas, przewijanie i dokładne zatrzymanie na końcu. Zasada obejmuje również szybki odczyt metadanych, więc lista i odtwarzacz korzystają z tej samej długości.

Stan `alpha.107`: komunikat Pomocy klawiatury ma stały układ **skrót → działanie → nazwa aktywnej sesji**. Kontekst nie powtarza bieżącego widoku ani rodzaju kontrolki, ponieważ działanie klawisza zostało już opisane wcześniej w tym samym komunikacie. Nazwą kontekstu jest dokładnie użytkowa nazwa sesji z przełącznika, np. `Pliki lokalne`, `WiiM`, `TIDAL` lub `Apple Music`. Lewa strzałka w sesji lokalnej jest opisana jako oznajmianie wielkości i bitrate pliku; w adapterach usług jako oznajmianie bitrate i innych dostępnych parametrów, bez obiecywania danych, których adapter nie udostępnia. Ten sam konkretny opis zastępuje ogólną „krótką informację” w spisie `F1`.

Stan `alpha.108`: wspólna osłona dekodera obejmuje MP3, MP2, WAV, AAC, M4A, FLAC, WMA, OGG/Vorbis, Opus i AIFF. Przed rozpoczęciem odtwarzania odrzuca nieprawidłową częstotliwość, liczbę kanałów, rozmiar zdekodowanego strumienia, absurdalny czas oraz niewiarygodną relację czasu do wielkości pliku. Odczyt pozycji dla paska i czytnika ekranu korzysta z nieblokującej wartości zapamiętanej, więc nie czeka na zajęty dekoder. Osobny nadzór trwającego odczytu zatrzymuje bieżący element po ośmiu sekundach bez odpowiedzi, zapisuje plik i przyczynę do logu oraz blokuje ponowną próbę tego samego źródła do restartu AMC. Nie zamyka aplikacji, nie modyfikuje i nie usuwa materiału. Szybkie informacje techniczne są odczytywane poza wątkiem interfejsu z pięciosekundowym limitem; ponawianie zapytania do pliku, który już przekroczył limit, jest blokowane w bieżącym uruchomieniu. Otwieranie odtwarzania nadal ma osobny limit 30 sekund dla pliku lokalnego i dwóch minut dla jawnie pobieranego pliku chmurowego.

Stan od `alpha.110`: lokalny folder jest kontenerem, a nie elementem multimedialnym przechowywanym w kolekcjach. Enter otwiera poziom folderu. Polecenia Kolejki, „odtwarzaj jako następne”, Ulubionych i playlist rozwijają zaznaczony folder rekurencyjnie do unikatowych, dostępnych utworów już obecnych w katalogu AMC. Nie sondują nośnika, nie czytają metadanych i nie wymuszają pobierania plików chmurowych. Kolejność wejściowa wynika ze ścieżek względnych w folderze. Dla przełączanych kolekcji obowiązuje semantyka kontenera: jeżeli choć jeden rozwinięty utwór należy do danej kolekcji, polecenie usuwa z niej całą pozostałą zawartość folderu; jeżeli nie należy żaden, dodaje wszystkie. Dzięki temu odtworzenie lub ręczne usunięcie pojedynczego pliku z Kolejki nie powoduje ponownego dodania folderu. Całość stanowi jeden wpis `Ctrl+Z`, który przywraca dokładny stan częściowy sprzed operacji. Komunikat podaje nazwę folderu i liczbę rzeczywistych plików. Pusty folder nie tworzy rekordu. Rdzeń dodatkowo odrzuca każdy sztuczny `MediaItemKind.Folder`, gdyby inny interfejs omyłkowo przekazał go bez rozwinięcia. `Ctrl+Shift+L` pozostaje operacją pliku, nie folderu źródłowego.

Stan `alpha.111`: operacja zmiany pozycji dekodera nigdy nie może zajmować wątku interfejsu. Zamontowany dostawca chmurowy może zablokować ustawienie pozycji podczas pobierania odległego fragmentu, dlatego odtwarzacz wykonuje skok w jednym pracowniku tła przypisanym do bieżącego toru. Kolejne żądania zastępują oczekujący cel; po zakończeniu wcześniejszego dostępu wykonywany jest tylko najnowszy skok. Do czasu jego zakończenia model i pasek zwracają żądaną pozycję, co pozwala liczyć następny skrót od celu i nie oddaje fokusu. Zmiana utworu, zatrzymanie albo zamknięcie unieważnia pracownika bez zastosowania wyniku do nowego toru. Zwykły ośmiosekundowy watchdog odczytu nie interpretuje świadomego doczytywania jako awarii; osobny limit przewijania wynosi dwie minuty. Diagnostyka zapisuje tytuł, cel i czas wolnego skoku. Typowe ścieżki strumieniowanego Dysku Google, iCloud, OneDrive i Dropbox są rozpoznawane także bez atrybutów Cloud Files, przy czym rozpoznanie nie otwiera treści pliku.
...
Routing OGG używa wyniku preflightu: Vorbis trafia do znormalizowanego NVorbis, a Opus lub inny strumień Ogg do ścieżki systemowej. NAudio 2.3.0 czyta WAV i AIFF własnymi parserami, w tym poprawiony RF64; MP3 używa Media Foundation z lokalnym fallbackiem NLayer; AAC/MP4, FLAC, WMA/ASF, Matroska/WebM i pozostałe zarejestrowane formaty korzystają z Media Foundation oraz kodeków dostępnych w danym Windows. AMC nie instaluje globalnego pakietu kodeków. Po wyjątku dowolnego dekodera bieżący tor jest atomowo odłączany i zwalniany, więc ponowienie zdalnego źródła tworzy świeży tor, a jednoznacznie uszkodzone źródło lokalne może zostać objęte kwarantanną do restartu. Ostatni dostawca próbek wymaga liczby podzielnej przez liczbę kanałów; NaN i nieskończoności są zerowane z jednym wpisem diagnostycznym. Kontrola jest liniowa względem małego bufora wyjściowego i nie tworzy kopii całego pliku.

Stan `alpha.114`: klasyfikacja chmury najpierw pobiera `FileAttributeTagInfo` przez uchwyt otwarty wyłącznie do punktu ponownej analizy, a następnie wywołuje `CfGetPlaceholderStateFromAttributeTag`. Nie żąda dostępu do danych i nie rozpoczyna hydratacji. Stan `PARTIAL` albo `PARTIALLY_ON_DISK` oznacza placeholder, natomiast błędna wartość `INVALID` nie może zostać omyłkowo potraktowana jako zestaw wszystkich flag. Systemowa klasyfikacja działa dla każdego dostawcy implementującego Windows Cloud Files API; rozpoznanie nazw korzeni, etykiet wirtualnych dysków, udziałów sieciowych i typowych segmentów Google Drive, iCloud, OneDrive, Dropbox, Box, pCloud, MEGA, Proton Drive, Nextcloud, ownCloud oraz Sync pozostaje warstwą konserwatywną. Indeksowanie nadal nie otwiera ładunku. Trzy kolejne klasy limitów odtwarzania i przewijania pozostają adaptacyjne, a powtarzany błąd tego samego zdalnego źródła wprowadza wykładniczy cooldown 10, 30, 90 i maksymalnie 300 sekund. Udany start kasuje licznik; lokalny plik nie podlega temu mechanizmowi.

Preflight MP3 rozróżnia zadeklarowany początek danych od rzeczywiście potwierdzonej pierwszej ramki. Wersje ID3 spoza 2–4 oraz niesynchroniczne bajty rozmiaru są ostrzegane i nie sterują skokiem. Prawidłowy tag może mieć setki megabajtów: odczytywany jest tylko dziesięciobajtowy nagłówek, po czym następuje bezpośredni seek i co najwyżej 1 MiB skanu. Zwykły MP3 wymaga dwóch zgodnych ramek; free-format wymaga trzech zgodnych ramek w równych odstępach, aby przypadkowy ciąg bajtów nie został uznany za audio. Dla dużego tagu, ostrzeżenia albo danych poprzedzających audio tworzony jest seekowalny widok podstrumienia od pierwszej potwierdzonej ramki. Widok używa 64-bitowych pozycji, nie kopiuje zawartości i może obsłużyć plik większy niż 4 GiB przez Media Foundation. Jeżeli ta droga zawiedzie, NLayer pozostaje ostatnią próbą tylko dla rozpoznanego lokalnego pliku do 512 MiB. Pierwsze rozmiary bloków RIFF/WAVE/AVI, AIFF, FLAC, OGG, MP4 i ADTS AAC są porównywane z fizycznym końcem bez przechodzenia po całym kontenerze. To walidacja ostrzegawcza: nietypowy, ale obsługiwany plik nadal może zostać zaakceptowany przez dekoder.

Wpis `watchdog.waitForFreezeRecovery` należy do NVDA i oznacza próbę odzyskania dostępu do nieodpowiadającej aplikacji lub obiektu dostępności; nie jest źródłem zawieszenia. Własny wpis AMC `ui-watchdog` jedynie rejestruje brak pulsu głównego okna po ośmiu sekundach i późniejsze odzyskanie odpowiedzi. Natomiast `decoder-watchdog` i `metadata-watchdog` identyfikują konkretny plik i chronią interfejs przed ścieżką dekodowania.

Okno **Informacje o wersji** podaje pełny numer, kanał wydania, architekturę oraz licencję, a także umożliwia skopiowanie tych danych. Odsyłacz do repozytorium otwiera się przez domyślną przeglądarkę dopiero po skonfigurowaniu oficjalnego adresu; wersja robocza nie zawiera wymyślonych ani pustych łączy. **Sprawdź aktualizacje** używa podpisanego mechanizmu opisanego w punkcie 13.5 i przed jego konfiguracją jednoznacznie informuje, że serwer wydań nie jest jeszcze dostępny.
...
Radio internetowe jest osobnym adapterem rdzenia i korzysta z tych samych sesji, Ulubionych, historii oraz poleceń transportowych. Implementacja `alpha.115` obsługuje bezpośrednie strumienie, M3U/M3U8, PLS i XSPF, metadane stacji oraz wyszukiwanie publicznego katalogu Radio Browser. Import M3U8 odróżnia zwykłą listę stacji od manifestu HLS zawierającego znaczniki `#EXT-X-`. Mechanizmy Free Radio pozostały wzorem zachowania, lecz odtwarzanie nie zostało przeniesione do procesu NVDA ani uzależnione od binarnego silnika Free Radio.

Stan `alpha.117`: bezpośredni strumień MP3 może używać współczesnej odpowiedzi HTTP albo starszego wiersza stanu Shoutcast/Icecast `ICY 200 OK`. Dekoder systemowy pozostaje pierwszym wyborem. Dopiero gdy odrzuci połączenie, ograniczony klient zgodności nawiązuje je ponownie, przyjmuje wyłącznie prawidłową odpowiedź audio, ogranicza rozmiar nagłówków i czas łączenia, usuwa bloki metadanych ICY, a ramki MP3 dekoduje zarządzanym NLayer. Nie istnieje lista wyjątków dla konkretnych stacji. HTTPS zachowuje zwykłą walidację certyfikatu, a nieobsługiwane kodowanie fragmentowe nie jest zgadywane.

Stan `alpha.118`: polecenia plikowe są zależne od adaptera. Sesja lokalna udostępnia otwieranie plików, folderów i zarządzanie Folderami Biblioteki. Sesja Radia zamiast nich pokazuje **Importuj stacje z playlisty…** i **Dodaj stację radiową…**. `Ctrl+O` zachowuje znaczenie „otwórz dane dla bieżącej sesji”: lokalnie wybiera audio, a w Radiu importuje lokalny M3U/M3U8, PLS, XSPF lub eksport Ulubionych VRadio JSON. Parser ma limit 4 MiB i 5000 pozycji, przyjmuje wyłącznie bezwzględne HTTP/HTTPS, normalizuje etykiety, usuwa duplikaty i nigdy nie loguje odrzuconego URL ani surowego JSON. Import nie uruchamia stacji. Każda nowa próba odtwarzania unieważnia i anuluje poprzednie rozwiązywanie playlisty oraz połączenie ICY, dzięki czemu szybka nawigacja nie zostawia równoległych, starych gniazd. Limit 25 sekund nadal kończy próbę komunikatem bez technicznego wyjątku. Ścieżka NLayer jest jawnie ograniczona do MIME MP3; AAC/AAC+ i OGG nie mogą zostać błędnie ogłoszone jako rozpoczęty MP3. HLS i stare ICY AAC wymagają właściwego dekodera tych formatów, a nie zgadywania ramek MP3.

Stan `alpha.119`: transmisje OGG/Vorbis otrzymują sekwencyjny dekoder sieciowy niezależny od lokalnej osi czasu pliku; test na żywym Radio Emaus potwierdza 48 kHz stereo. Starszy MP3 ICY buforuje najwyżej 8 KiB początku i rozpoczyna NLayer dopiero przy dwóch kolejnych zgodnych ramkach. Jest to ochrona przed rozpoczęciem połączenia w połowie ramki oraz fałszywymi synchronizacjami w losowych bajtach. Resolver może podać kontrolowany wariant zgodności przed źródłem katalogowym: dotyczy znanych portów MP3 Programu 1, Programu 2, Czwórki i PR24 oraz bezpośredniego MP3 dla nieaktualnego manifestu Eski. Nie jest to ogólny mechanizm zgadywania adresów; mapowanie jest ograniczone do rozpoznanych dostawców i ma test regresji. Nieznane HLS i ICY AAC pozostają jawnie nieobsługiwane do czasu wdrożenia osobnego, aktualizowalnego komponentu dekodera. Próba nie przechodzi przez blokujący dekoder surowego AAC i nie może zatrzymać interfejsu.

Stan `alpha.120`: odczyt z gniazda w ścieżce ICY MP3 ma semantykę ramki, a nie pojedynczego pakietu TCP. Warstwa wejściowa dopełnia żądany fragment z kolejnych części sieciowych aż do uzyskania całej porcji albo rzeczywistego zamknięcia połączenia. Dotyczy to również granicy pomiędzy początkowym buforem wyrównującym a dalszym strumieniem. NLayer nie otrzymuje więc krótkiego odczytu udającego niepełną ostatnią ramkę. Test lokalny dzieli dane co 257 bajtów, a testy na żywo wykonują wiele kolejnych odczytów czterech anten Polskiego Radia. Zwykłe zerwanie połączenia nadal kończy tor kontrolowanym błędem; poprawka nie tworzy nieskończonej pętli ani pełnej kopii transmisji.

Stan `alpha.121`: resolver zna pary AAC+→MP3 Polskiego Radia dla Trójki `8954→8904` i Radia Chopin `8960→8910`, a także wariant HLS Trójki przez host `stream13`. Są to jawne pary tego samego nadawcy, nie heurystyka dowolnych portów. Test gniazdowy potwierdził działający MP3 Radia Chopin 192 kb/s. W tej samej chwili oba starsze wejścia Trójki zwracały `ICY 401 Service Unavailable`, zatem awaria nie może zostać naprawiona po stronie klienta bez użycia innego, np. HLS/AAC, źródła. Przygotowanie toru pobiera teraz pierwszą porcję dekodowanego PCM przed uruchomieniem WASAPI i zdarzeniem rozpoczęcia; usuwa to fałszywy stan „gra” dla samych nagłówków lub natychmiastowego końca źródła oraz wstępnie zasila timeshift.

Rozstrzygnięcie `alpha.126`: skrót nigdy nie może niejawnie zmienić adaptera i dopiero potem wykonać polecenia właściwego dla innej usługi. `Alt+1–3`, `F5` i `Ctrl+F5` są poleceniami układów i źródeł Biblioteki lokalnej, dlatego poza sesją Pliki lokalne są przechwytywane bez zmiany sesji, nie występują w jej menu kontekstowym i nie są przedstawiane przez Pomoc klawiatury jako aktywne. Przyszłe radiowe `Alt+1–3` wymagają osobnej decyzji o widokach; nie wolno odziedziczyć lokalnej semantyki. Import radia rozróżnia **nowy URL**, **istniejący wpis katalogowy poza Biblioteką** oraz **duplikat już obecny w Bibliotece**. Drugi przypadek włącza istniejący obiekt i zachowuje bogatsze metadane, dzięki czemu ponowny import tej samej listy naprawia pominięte wcześniej stacje bez dublowania rekordów.

Transmisje HLS z wieloma ścieżkami, w tym obrazem H.264 i dźwiękiem AAC, są dekodowane poza dispatcherem przez opcjonalny proces FFmpeg, który przekazuje wyłącznie ograniczony format PCM do istniejącego timeshiftu, nagrywania i WASAPI. Argumenty procesu nie są składane jako polecenie powłoki, prywatny URL nie trafia do logu, anulowanie zmiany stacji zamyka potok i proces, a brak `ffmpeg` kończy się zwykłym błędem stacji. Bieżąca alfa wykrywa plik obok EXE, zmienną `FFMPEG_PATH` i `PATH`; docelowa dystrybucja wymaga osobnego, aktualizowalnego pakietu o ustalonej konfiguracji i licencji. Test rzeczywisty potwierdził zwracanie dźwięku z HLS Jarosławia i św. Jana Vianneya. Źródło będące stroną YouTube nie jest HLS ani bezpośrednim radiem i wymaga przyszłego adaptera rozwiązywania URL; nie wolno przekazywać go losowo dekoderom audio.

Bitrate radia ma trzy podstawowe źródła: metadane katalogu, parametr raportowany przez faktycznie otwarty dekoder oraz ograniczoną próbę wykonaną na jawne żądanie lewej strzałki. Próba sprawdza nagłówki ICY, potwierdzone ramki MP3, ramki ADTS AAC, pasmo wariantu audio HLS i rozmiar jednoznacznie dźwiękowego segmentu. Manifest z kodekiem obrazu nie może podać całkowitego pasma wideo jako bitrate radia. Gdy chroniony lub wadliwy serwer uniemożliwia pomiar, jawnie utrzymany profil zgodności konkretnego nadawcy może podać wartość awaryjną oznaczoną jako „około”; nie jest ona przedstawiana jako wynik odczytu. Wykryte `kb/s` i częstotliwość próbkowania są zapisywane przy stacji i pojawiają się w krótkiej informacji oraz pasku. Dla nieznanego HLS bez deklarowanego pasma wartość pozostaje nieznana, a bitrate wyjściowego PCM lub MP3 nagrania nie może udawać bitrate źródła. Pasek WinForms zachowuje systemową rolę, lecz nie ma dodatkowej nazwy dostępnościowej; dynamiczną treść udostępnia tylko jego etykieta. Dzięki temu `NVDA+End` odnajduje stan bez wstępu „Pasek stanu odtwarzania”. W Radiu tytuł okna zaczyna się od nazwy stacji i może dopisać bieżący utwór lub audycję wyłącznie z metadanych aktywnego strumienia. Tekst jest ulotny, ograniczony, oczyszczany, nie zmienia nazwy stacji i zostaje wyzerowany przed przełączeniem anteny.

Zabezpieczenie `alpha.128`: adapter BASS używa osobnych stałych dla bitrate i częstotliwości. Odczyt `BASS_ATTRIB_BITRATE` zwraca kilobity na sekundę, natomiast częstotliwość pochodzi z formatu PCM. Niemożliwy radiowy bitrate powyżej 10 000 kb/s jest odrzucany na wejściu, przy odczycie starego stanu i przed ponownym zapisem. Usuwa to historyczne `44100 kb/s` bez kasowania poprawnego `44,1 kHz`. Import playlisty jest operacją włączenia stacji do Biblioteki: zgodność jest ustalana po dokładnym, kanonicznym URL, a istniejąca nazwa i metadane mają pierwszeństwo. Nie wolno scalać wyłącznie po podobnej nazwie, ponieważ jedna rozgłośnia może mieć osobne anteny, kodeki i przepływności.

Rozstrzygnięcie `alpha.129`, uzupełnione w `alpha.135`: **Radio internetowe** udostępnia Bibliotekę stacji, Ulubione, Historię, playlisty stacji, wyszukiwanie, informacje, timeshift i nagrywanie. Nie udostępnia Kolejki utworów, „Odtwórz jako następne”, Albumów ani Zakładek. Niedostępne działania są usuwane z menu głównego, menu kontekstowego listy i odtwarzacza, wyników wyszukiwania oraz palety poleceń, a router odrzuca również ich skróty. Stary zapis Kolejki radia nie jest odtwarzany ani ponownie zapisywany. Jednorodna lista stacji pomija nazwę rodzaju `stacja`, ponieważ rolę i pozycję podaje sama kontrolka listy. Manifest HLS jest źródłem nadrzędnym; jawnie powiązany, starszy adres MP3 jest próbą awaryjną, nie zamiennikiem wykonywanym przed HLS.

Ręczne porządkowanie Ulubionych radia ma dwa tryby. `Alt+strzałka w górę/w dół` przesuwa ciągły blok o jedną pozycję i służy do drobnych korekt. `Ctrl+X` zapisuje stabilne identyfikatory aktualnie zaznaczonych stacji, użytkownik przechodzi zwykłymi strzałkami do miejsca docelowego, a `Ctrl+V` atomowo umieszcza cały blok przed wybraną stacją. Operacja nie dotyka schowka plikowego, nie zmienia adresów strumieni i powoduje jedno odświeżenie listy. Filtr musi być pusty, aby ukryte wiersze nie czyniły miejsca docelowego niejednoznacznym.

AMC rozróżnia **własne presety każdej sesji** od **presetów urządzenia** WiiM, BluOS lub przyszłego adaptera sprzętowego. Każda sesja ma dwanaście niezależnych miejsc `1–0`, `-` i `=` przechowywanych przez AMC. `Ctrl+Shift+1–0/-/=` natychmiast uruchamia zajęte miejsce bieżącej sesji, a puste niczego nie zmienia i wyjaśnia sposób przypisania. `Ctrl+Alt+P` otwiera dostępną listę wyłącznie do uruchamiania: Enter i Spacja włączają zajęty preset i nigdy go nie zapisują ani nie nadpisują. Lista pozwala zaznaczyć kilka miejsc Shiftem; `Ctrl+C` kopiuje nazwy zajętych elementów, a `Ctrl+Shift+C` nazwy i dostępne ścieżki lub łącza, z pominięciem pustych miejsc. `Ctrl+Alt+Shift+P` otwiera tryb przypisania aktualnego albo zaznaczonego elementu, podaje pierwsze wolne miejsce, przyjmuje pojedynczy klawisz miejsca, a Enter zapisuje. Zajęte miejsce wymaga ponownego wskazania tego samego klawisza i dopiero Entera; wybór innego miejsca zmienia cel, a Escape anuluje całą operację. Delete przygotowuje usunięcie wybranego przypisania, które również wymaga Entera. Dzięki temu zwykłe wywołanie presetu pozostaje natychmiastowe, a zapis nie uruchamia najpierw poprzedniego celu. Długie przytrzymanie nie jest używane, ponieważ powtarzanie klawisza, układ klawiatury i współpraca z czytnikiem ekranu czynią je mniej przewidywalnym.

Na liście presetów pojedynczy klawisz miejsca `1–0`, minus albo znak równości wyłącznie przenosi fokus do odpowiadającej pozycji; nie uruchamia jej i nie zmienia przypisania. Numeracja listy pozostaje 1–12, dlatego klawisz `0` wskazuje miejsce 10. Bezpośrednie wywołanie pustego miejsca posługuje się natomiast etykietą fizycznego klawisza: `Ctrl+Shift+0` mówi „Preset 0 pusty”. Skróty bezpośrednie są obsługiwane również na poziomie komunikatów klawiatury Windows, aby standardowe przetwarzanie WPF albo czytnik ekranu nie mogły zgubić zera.

Przypisanie lokalnego presetu utrwala stabilne wskazanie stacji i w razie potrzeby zachowuje ją w Bibliotece radia; wyczyszczenie presetu nie usuwa stacji z Biblioteki ani Ulubionych. Preset AMC nie wymaga przynależności celu do Biblioteki, Ulubionych, Kolejki ani playlisty: przechowuje sesję, rodzaj zasobu, stabilny identyfikator i oficjalne łącze awaryjne, dzięki czemu może wskazywać stację, utwór, album, playlistę albo dynamiczny miks. `Ctrl+Alt+P` jest wspólną listą presetów aktywnej sesji, `Ctrl+Alt+Shift+P` tworzy lub przypisuje preset, a `Ctrl+Shift+1–0/-/=` natychmiast uruchamia odpowiednie miejsce. `Ctrl+P` i `Ctrl+Shift+P` zawsze zachowują spójne znaczenie playlist i działają również na stacjach radiowych. Presety urządzenia pozostają inną kolekcją należącą do konkretnego urządzenia i są odczytywane lub zapisywane przez jego API; AMC nie synchronizuje ich pod tą samą tożsamością z lokalnymi presetami programu.

W bieżącej implementacji **Plików lokalnych** preset może wskazywać plik, dowolny widoczny folder z zarejestrowanego źródła Biblioteki, rozpoznany album albo playlistę AMC. Plik jest identyfikowany trwałym identyfikatorem katalogu AMC i po wywołaniu od razu się odtwarza; folder zachowuje pełną ścieżkę, a album i playlista otwierają swoją zawartość bez samoczynnego uruchamiania dźwięku. Ta sama lista i przypisanie działają w Folderach, Wszystkich plikach, Kolejności własnej, Ulubionych, wynikach wyszukiwania i odtwarzaczu. Preset nie przełącza sam sesji. Gdy cel zniknie z aktywnej Biblioteki albo folder zostanie odłączony, przypisanie pozostaje zapisane i jest oznajmiane jako niedostępne, aby czasowa niedostępność chmury nie usuwała danych użytkownika.

W bieżącej implementacji Radia preset może wskazywać jedną stację albo playlistę stacji AMC. Stacja uruchamia się natychmiast, a playlista otwiera się jako zwykły widok; użytkownik wybiera z niej antenę Enterem. `Page Up` i `Page Down` przechodzą potem wyłącznie po stacjach kontekstu, z którego rozpoczęto odtwarzanie. Playlista radia nie jest folderem odtwarzanym automatycznie do końca: transmisja na żywo nie ma naturalnego końca, więc zmiana stacji pozostaje poleceniem użytkownika.

Planowany adapter YouTube dla Radia przechowuje wyłącznie kanoniczny adres filmu, kanału lub transmisji oraz użytkową nazwę. Podpisanego, krótkotrwałego adresu audio nie zapisuje w Bibliotece: rozwiązuje go dopiero przed odtwarzaniem albo rozpoczęciem zaplanowanego nagrania i odświeża po wygaśnięciu. Harmonogram wskazuje stabilny wpis AMC, ma ograniczone ponowienia przed i po planowanym czasie oraz zapisuje tylko bezpieczny wynik diagnostyczny bez ciasteczek i podpisanych URL. Resolver jest opcjonalnym, aktualizowalnym procesem odseparowanym od rdzenia; brak lub awaria nie może uszkodzić zwykłego radia. Oficjalne metadane i wyszukiwanie pozostają oddzielone od eksperymentalnego pozyskania strumienia audio, a użycie konta lub ciasteczek wymaga osobnej, świadomej zgody użytkownika.

Stan `alpha.122`: zwolnienie anulowanego toru może równolegle przerwać blokujący odczyt `BufferedStream` wyjątkiem `NotSupportedException`. Przy ustawionym znaczniku anulowania jest to oczekiwane zakończenie, tak jak zamknięcie obiektu lub błąd wejścia-wyjścia. Bez anulowania ten sam typ nadal odłącza bieżący tor i tworzy kontrolowany komunikat o przerwaniu. Zadanie przechwytujące nie pozostawia wyjątku dla finalizatora i nie konkuruje z nową stacją.

Stan `alpha.123`: po przerwaniu działającego strumienia tor odbioru wykonuje najwyżej dwie automatyczne próby ponownego połączenia, a każda przechodzi przez tę samą uporządkowaną listę wariantów adresu co pierwszy start. Powrót wymaga pierwszej prawidłowo zdekodowanej porcji PCM. Zgodny format pozwala atomowo zastąpić tylko czytnik sieciowy, zachowując bufor timeshift, WASAPI i rozpoczęte nagranie; zmiana częstotliwości, kanałów, kodowania, rozmiaru próbki albo wyrównania bloku jest odrzucana, ponieważ wymagałaby przebudowy całego toru. Dwadzieścia sekund ciągłego odbioru zeruje licznik. Zmiana stacji i zamknięcie nadal natychmiast anulują oczekiwanie oraz nie mogą zostać cofnięte przez spóźnione połączenie.
...
Odtwarzacz odbiera i dekoduje transmisję poza wątkiem okna, a dekodowany dźwięk umieszcza w ograniczonym pierścieniowym buforze pamięci. Strzałki lewo/prawo przemieszczają się po timeshifcie, Home wybiera najstarszy nadal dostępny fragment, End wraca na żywo, a odczyty czasu rozróżniają pozycję w buforze, opóźnienie względem transmisji i długość dostępnego bufora. Bufor nie jest trwałym archiwum i znika po zatrzymaniu stacji. W odtwarzaczu radia cyfry `0–9` wybierają `0%, 10%, …, 90%` aktualnie dostępnego bufora, również po nadpisaniu jego najstarszej części. `0` oznacza najstarszy dostępny fragment, `5` środek, a `9` pozycję blisko transmisji na żywo; `End` nadal wraca na żywo. Skok zachowuje pauzę i wybrane tempo. Procent nie odnosi się do ustawionego limitu bufora ani całego czasu działania stacji. Pusty lub jeszcze przygotowywany bufor jest zgłaszany komunikatem. Ustawienia odczytu procentu/czasu i jego wyciszenia działają tak jak dla plików; czas jest liczony od początku dostępnego bufora. Wpisywanie czasu lub procentu w dialogu oraz zakładki do samej transmisji pozostają niedostępne; regulacja tempa dotyczy zapasu dźwięku w buforze.

Nagrywanie radia jest świadomie uruchamianą funkcją lokalną do prywatnego użytku: `R` w odtwarzaczu oraz `Ctrl+Alt+R` na liście zapisują od chwili włączenia wybrany strumień do wskazanego formatu i folderu. Od `alpha.153` każde nagranie ręczne używa własnego, niesłyszalnego toru, dlatego zmiana słuchanej stacji nie kończy zapisu, a różne stacje mogą być nagrywane równolegle. `T` finalizuje bieżącą część aktywnego nagrania i uruchamia następny plik na tym samym torze, zachowując ewentualną pauzę; od `alpha.295` obejmuje to również bieżące nagranie z harmonogramu, bez wyłączania planu i bez zmiany przyszłych terminów. Punkty pauzy pozostają przypisane do części, w której powstały. Od `alpha.201` zakończona część natychmiast trafia do lokalnej Biblioteki, a ponowne `T` w ciągu pierwszych pięciu sekund nowej części jest pomijane bez zatrzymywania całego nagrania. `Shift+T` nie jest skrótem podziału. Nagranie kończy ponowne polecenie dla tej samej stacji albo kontrolowane zamknięcie aplikacji. `Alt+2` w Radiu otwiera ulotny widok aktywnych nagrań ręcznych i planowych; widok nie jest przywracany po ponownym uruchomieniu. Domyślnym wynikiem jest MP3 kodowany przez systemowy Windows Media Foundation, z jakością docelową 192 kb/s. MP3, AAC, OGG i inne obsługiwane źródła mogą przejść przez dekodowany tor PCM; wariant Oryginalny kopiuje pakiety w drugim połączeniu FFmpeg. Zapis odbywa się do pliku roboczego `.amc-partial`, a docelowa nazwa powstaje atomowo dopiero po zamknięciu kodera. Awaria usuwa niedokończone dane, jest oznajmiana bez identyfikatora technicznego i nie zatrzymuje odsłuchu. Funkcja nie uruchamia się automatycznie, nie dotyczy TIDAL, Spotify ani Apple Music, nie obchodzi DRM i pozostawia użytkownikowi odpowiedzialność za zgodność z prawem właściwym dla miejsca użycia.
...
Operacyjna lista składników, wymagane testy FFmpeg oraz zasady oddzielnych
paczek dodatków NVDA są utrzymywane w `AKTUALIZACJE_KOMPONENTOW.md`.
...
Korekta `alpha.195` traktuje jawnie otwartą listę `Ctrl+H` jako widoczny kontekst odtwarzania, a nie ukrytą playlistę: uruchomienie z niej elementu powoduje, że `Page Up/Page Down` i naturalny koniec pliku przechodzą po aktualnej kolejności Historii. Tymczasowa nawigacja `Alt+strzałka w górę/w dół` wykonana już w odtwarzaczu nadal zachowuje kontekst, z którego odtwarzacz został wcześniej otwarty. Zakładki i wyniki wyszukiwania pozostają lokalizatorami.
...
Korekta `alpha.99`: „Odtwórz jako następne” nie jest osobnym widokiem ani drugą trwałą kolekcją, lecz priorytetową częścią jednej Kolejki. `Ctrl+Q` podaje liczność obu części, a etykieta każdego priorytetowego wiersza zawiera krótki prefiks „Następny”. Rdzeń utrzymuje osobny, chwilowy kontekst nawigacji aktywnej Kolejki. Powstaje on zarówno po jawym uruchomieniu pozycji z `Ctrl+Q`, jak i po automatycznym wejściu do Kolejki z innego kontekstu. Dzięki temu Page Up/Page Down nie wraca przypadkowo do porządku Biblioteki. Od `alpha.208` bieżąca pozycja pozostaje widoczna w Kolejce podczas odtwarzania i jest zużywana dopiero po naturalnym zakończeniu albo ręcznym przejściu dalej; chwilowy kontekst nadal pozwala Page Up wrócić. Zużyte pozycje nie są ponownie wybierane przez automatyczną kontynuację. Po opróżnieniu priorytetowej i zwykłej części AMC albo wraca do zapamiętanego kontekstu źródłowego, albo kończy jawną Kolejkę.
...
Korekta dostępności `alpha.102`: po otwarciu **Folderów Biblioteki** fokus trafia na kontener pierwszego zaznaczonego wiersza, nie na samą kontrolkę listy. Dostępnościowa etykieta oddziela osiągalność korzenia („folder dostępny” albo „folder niedostępny — rekordy zachowane”) od liczników rekordów. Aktywny rekord należy do Biblioteki i jego ścieżka jest widoczna; chmurowy placeholder jest aktywny bez wymuszania pobrania danych. Niedostępny rekord był wcześniej znany, lecz jego ścieżki chwilowo nie można osiągnąć. Wykluczony rekord został usunięty z Biblioteki przez `Delete` i jest celowo pomijany przez synchronizację, mimo że fizyczny plik może nadal istnieć.

Zasada porządkowania z `alpha.103`: niedostępność nigdy nie powoduje automatycznego usunięcia rekordu. **Foldery Biblioteki** udostępniają dla wybranego folderu osobną, wielokrotnie zaznaczaną listę niedostępnych plików. Operacja **Zapomnij w AMC** wymaga ostrzeżenia z domyślną odpowiedzią „Nie” i usuwa atomowo rekord katalogu oraz jego identyfikator z Ulubionych, Kolejki, playlist, Historii, Zakładek, kolejności własnej, kontekstu odtwarzania i zapamiętanej pozycji. Nie wywołuje operacji systemu plików i nie zatrzymuje bieżącego odtwarzania. Placeholder widoczny dla systemu nie kwalifikuje się do zapomnienia. Jeżeli zapomniany plik pojawi się później, skan tworzy dla niego nowy rekord zamiast odtwarzać usunięte relacje.

Semantyka Historii z `alpha.104`: `Ctrl+H` otwiera trwałą, właściwą dla bieżącej sesji listę unikatowych elementów od najnowszego. Jest to pełnoprawna lista wyboru i źródło elementów dla kopiowania, Kolejki, Ulubionych i zarządzania playlistami, lecz nie własna kolejka odtwarzania ani ręcznie sortowana kolekcja. `Delete` usuwa atomowo tylko wybrane identyfikatory z Historii tej sesji i nie zmienia żadnej przynależności ani pliku. Usuwanie z Biblioteki jest dostępne wyłącznie w widokach składających się na Bibliotekę; usuwanie z Kolejki i playlisty analogicznie dotyczy tylko aktywnego widoku. `Shift+Delete` zachowuje odrębną semantykę systemową i dla istniejącego pliku lokalnego dopiero po potwierdzeniu przenosi go do Kosza.
...
Prosty montaż audio jest etapem późniejszym po Zakładkach i Folderach. Pierwszy zakres obejmie niedestrukcyjne punkty A–B, odsłuch zaznaczenia i zapis fragmentu jako nowego pliku. Następnie lista fragmentów pozwoli utworzyć nowy plik z kilku źródeł. Oryginały nie będą nadpisywane. Bezstratne cięcie i łączenie będzie używać dojrzałych narzędzi właściwych dla formatu; ponowne kodowanie musi być jawne, a operacja zapisywana przez plik tymczasowy i atomowe ukończenie. Niewielka funkcja rozdziałów wspólnego odtwarzacza wykorzystuje nazwaną Zakładkę `Ctrl+Shift+B` jako punkt wyjścia, ale nie zmienia każdej zakładki w rozdział. Podcasty dziedziczą tę funkcję wraz z odtwarzaczem; opis znajduje się w `PROJEKT_ROZDZIALOW_AUDIO_PL.md`.
...
Semantyka playlist od `alpha.140`: kontener i jego zawartość są dwoma różnymi poziomami. Enter otwiera zawartość, `Ctrl+Enter` uruchamia pierwszy dostępny element i zapisuje całą nieprzefiltrowaną playlistę jako kontekst odtwarzania. Działania na Kolejce, „Odtwórz jako następne”, Ulubionych i Bibliotece wykonywane na nazwie playlisty są działaniami zbiorczymi na jej zawartości i muszą tak być nazwane; nie tworzą niejawnego znacznika ulubionej playlisty. `Alt+Enter` opisuje kontener. Lista skończonych materiałów podaje łączny czas, częściowe dane oznacza jako częściowe, a lista stacji mówi „transmisje na żywo”, ponieważ nie ma skończonej długości. W Radiu Kolejka i „Odtwórz jako następne” są wyłączone również na kontenerze. Lokalne `Alt+1/2/3` może opuścić wnętrze playlisty i przejść do jednego z trzech widoków Biblioteki.
...
Uzupełnienie `alpha.142`: aktywne okno AMC przechwytuje dokładnie `Ctrl+Shift+0` także w niskopoziomowym haku klawiatury, zanim kombinacja dotrze do WPF. Jest to celowe obejście dodatków czytnika ekranu pochłaniających skrót; nie jest globalnym presetem i nie działa, gdy AMC jest w tle. `Ctrl+0` zachowuje wyłącznie listę sesji. Nagrywanie ręczne działa przełącznikiem `R` w odtwarzaczu oraz `Ctrl+Alt+R` na liście Radia. `Shift+R` na stacji otwiera bezpośredni dialog nagrania czasowego: tryb domyślny rozpoczyna niezależne nagranie natychmiast na podaną liczbę minut, a po jego wyłączeniu dostępny jest przyszły termin i reguła powtarzania. Polecenia oraz menedżer planów muszą być widoczne w adekwatnych menu kontekstowych. Od `alpha.153` ręczne nagranie jest niezależne od odsłuchu, więc zmiana stacji nie wyświetla modalnego pytania i nie przerywa zapisu; użytkownik kończy je jawnie dla nagrywanej stacji.

Uzupełnienie `alpha.143`: ogólne ustawienia Radia obejmują tryb folderu nagrań, format `MP3`, `M4A/AAC` albo `WAV`, bitrate 96–320 kb/s dla kodowania stratnego i domyślne wybudzanie. Wpis planu zachowuje pustą ścieżkę wyłącznie jako jawne dziedziczenie folderu ogólnego; własny folder jest pełną ścieżką i może zostać wybrany dostępnym selektorem. Analogicznie wartość wybudzania `null` znaczy dziedziczenie, lecz dostępna etykieta musi powiedzieć, jaka wartość ogólna jest obecnie dziedziczona. Przed nagraniem folder przechodzi rzeczywistą próbę zapisu, a łańcuch awaryjny to: folder planu, folder ogólny, systemowy `Muzyka\AMC — Nagrania radia`. Żadna nazwa klasy, enum ani surowa wartość nie może trafić do NVDA.

Nagrywanie odbywa się za dekoderem, ze wspólnego PCM, więc źródłowy kontener nie określa formatu wyniku. Ta architektura obejmuje HLS z dźwiękiem wyodrębnianym przez FFmpeg, wszystkie dekodery systemowe, OGG i BASS, a ręczny odsłuch oraz prywatny tor harmonogramu mają te same zasady rozwiązywania adresów i ponawiania połączenia. Nie dodajemy trybu „oryginalny” dla HLS: kopiowanie zmiennych segmentów, kluczy, map inicjalizacyjnych i wariantów adaptacyjnych byłoby odrębnym rejestratorem oraz inną klasą ryzyka. Koder publikuje wynik atomowo dopiero po finalizacji; błędny lub niedokończony plik roboczy nie może udawać nagrania.

Korekta `alpha.144`: bezpośredni edytor `Shift+R` otrzymuje uporządkowany zbiór z aktualnie widocznych wierszy listy, a nie wszystkie obiekty sesji. Jest to ważne, ponieważ adapter Radio Browser celowo przechowuje pobrane wyniki w katalogu technicznym do ponownego wykorzystania. Menedżer wszystkich planów rozszerza zakres wyłącznie o Bibliotekę, Ulubione, bieżącą stację i stabilne odwołania istniejących planów. Widokowy zakres oraz zaznaczenie są testowane niezależnie od UI Automation.

Wyjątek doprecyzowany w `alpha.289`: **Nagrywane** jest tymczasowym widokiem stanu, a nie trwałą kolekcją stacji. `Shift+R` uruchomione w tym widoku zachowuje wskazaną nagrywaną stację jako wybór początkowy, lecz selektor stacji obejmuje całą Bibliotekę Radia. Nie może zostać ograniczony do stacji aktualnie nagrywanych ani rozszerzony o ukryte, nieużywane wyniki katalogu Radio Browser. Pozostałe stabilne widoki nadal przekazują swoją aktualnie widoczną, sensowną zawartość.

Stan aktywnego zapisu jest relacją identyfikatora stacji i, awaryjnie, dokładnego adresu strumienia. Wspólne formatowanie list dodaje po nazwie stacji jeden stan `nagrywanie`, `nagrywanie wstrzymane` albo `część nagrań wstrzymana`; nie powiela czasu ani ścieżki w każdym wierszu. Ręczne nagranie przechowuje w pamięci chwilę uruchomienia, format i docelowy plik, a aktywny plan także rzeczywistą chwilę startu, deadline i planowany folder. Te dane są prezentowane w sekcji Nagrywanie istniejącego `Alt+Enter`, po czym znikają atomowo z etykiet po zakończeniu albo błędzie.
...
Nagrywanie od `alpha.146` rozróżnia kodowanie z PCM od kopiowania pakietów źródłowych. MP3, AAC i FLAC są koderami zasilanymi zdekodowanym PCM, a WAV zapisuje sam PCM. Wariant Oryginalny tworzy osobne połączenie FFmpeg i używa `copy`, dlatego nie zmienia stratnie kodeka; wybór kontenera zależy od rozpoznanego kodeka i rodzaju źródła. HLS jest przepakowywany do audio-only MPEG-TS, a nie zapisywany jako manifest. Plik końcowy nadal powstaje atomowo z `.amc-partial` dopiero po poprawnej finalizacji. Brak FFmpeg, błąd demultipleksera, zerowy plik albo nieudane zamknięcie usuwa plik roboczy i nie zatrzymuje odsłuchu. Zapis oryginalny może utworzyć drugie połączenie do serwera stacji i nie gwarantuje zachowania metadanych tekstowych niezwiązanych z dźwiękiem.

Zasada folderów nagrań od `alpha.147`: ustawienia programu przechowują dokładnie jedną jawną, bezwzględną ścieżkę domyślnego folderu nagrywania. Interfejs nie rozdziela jej na techniczne warianty „systemowy” i „własny”; pokazuje ścieżkę oraz przycisk wyboru folderu. Dawna pusta wartość jest przy odczycie rozwiązywana do `Muzyka\AMC — Nagrania radia`, co zachowuje zgodność danych. Harmonogram przechowuje pustą ścieżkę wyłącznie jako znacznik dziedziczenia albo pełną ścieżkę wyjątku dla danego planu. Jego dostępne pole kombi musi zawierać użytkowe etykiety „Domyślny folder nagrywania” oraz „Inny folder”, bez nazw typów, enumów i technicznych identyfikatorów. Zmiana wartości domyślnej działa dynamicznie na wszystkie plany dziedziczące; plan z wyjątkiem pozostaje niezależny. Niedostępny cel nadal podlega bezpiecznej regule awaryjnej i nie może utracić nagrania tylko dlatego, że odłączono dysk lub chmurę.

Zasada edycji harmonogramu od `alpha.148`: dni tygodnia są jednym, pojedynczym punktem tabulacji. Lista porusza się strzałkami, Spacja przełącza niezależny stan dnia, a dostępna nazwa każdego wiersza zawsze zawiera użytkową nazwę dnia i „zaznaczony” albo „niezaznaczony”. Stan zaznaczenia nie może być utożsamiany z bieżącym wierszem listy. Lista planów analogicznie przełącza pole `Enabled` Spacją bez otwierania edytora; wyłączenie nie usuwa rekordu. Zmiany w oknie zarządzania są transakcyjne i wpływają na główny harmonogram dopiero po Zapisz. Szybkie rozpoczęcie oznacza pierwszy start natychmiast po zatwierdzeniu, wymusza włączenie planu i pomija datę oraz godzinę pierwszego wystąpienia. W planie cyklicznym rzeczywista godzina tego startu staje się godziną kolejnych wystąpień. Użytkowy wyjątek folderu nosi etykietę „Folder użytkownika”; wewnętrznie nadal jest pełną ścieżką, a pusta wartość oznacza dziedziczenie.

Zasada terminu i fokusu od `alpha.149`: nowy plan rozdziela wybór **Od razu po zapisaniu** od **W wybranym terminie** w jednym polu kombi. Data i godzina są natywnymi, segmentowymi polami Windows; lewo i prawo wybiera część, góra i dół zmienia wartość, a kontrolka sama zachowuje prawidłową datę kalendarzową. Długość jest ograniczonym polem liczbowym. Model nadal przechowuje UTC wraz z identyfikatorem strefy czasowej, nigdy tekst widoczny w kontrolce. Tab porusza się po funkcjach, a nie po technicznych fragmentach hosta. Escape i Enter muszą działać także wewnątrz kontrolek hostowanych. Nowy plan otrzymuje fokus na stacji, edytowany na dacie, a zamknięcie edytora przywraca fokus zaznaczonemu wierszowi listy planów. Żadna kontrolka nie może ujawnić klasy, rekordu, enumu ani nazwy właściwości.

Doprecyzowanie `alpha.290`: po zmianie wartości strzałką góra albo dół pole daty zawsze dołącza dzień tygodnia. Segment dnia podaje zwięźle dzień i miesiąc cyframi, np. „23.09, środa”; segment miesiąca podaje dzień i pełną nazwę miesiąca, np. „4 września, piątek”; segment roku podaje pełną datę z rokiem i dniem tygodnia. Lewo i prawo nadal służy wyłącznie do wyboru segmentu i wtedy podaje jego nazwę. Dzięki temu szybka zmiana daty nie wymaga osobnego odczytywania całej kontrolki.

Zasada zwięzłego odczytu WiiM od `alpha.291`: zmiana strumienia lub presetu nie może powodować automatycznego odczytania całego modelu urządzenia. Dostępna nazwa skupionego przycisku transportu zawiera nazwę bieżącego materiału oraz działanie, np. „357. Odtwórz” albo „357. Wstrzymaj”. Nie powtarza słów „Odtwarzacz WiiM”, pełnej nazwy urządzenia, źródła, wartości głośności ani stanu nieznanego. Stan wyciszenia pozostaje istotny i jest podawany. Bezpośrednie uruchomienie presetu zachowuje numer presetu oraz przypisany skrót. Pełny model, głośność i parametry pozostają w widocznych polach odtwarzacza, pasku stanu oraz informacjach wywoływanych na żądanie.
...
Zmiana `alpha.153`: ręczne nagrania radia działają w prywatnych, niesłyszalnych torach i są identyfikowane niezależnie od bieżącego odtwarzacza. Zmiana stacji nie otwiera pytania modalnego, nie przerywa mowy NVDA i nie finalizuje nagrania. `Alt+2` w sesji Radia pokazuje listę **Nagrywane**, `Ctrl+Alt+R` steruje nagraniem stacji z listy, a `R` pozostaje najkrótszym przełącznikiem w odtwarzaczu oraz na liście Nagrywane. Widok łączy nagrania ręczne i aktywne plany, lecz zatrzymanie planu nadal odbywa się przez Harmonogram. Kopiowanie oryginalnego strumienia nie wymusza już indeksu `0:a:0`; FFmpeg wybiera właściwą ścieżkę dźwiękową, co obejmuje bezpośrednie serwery ICY, które przed rozpoczęciem odbioru nie publikują typowanego indeksu audio.

Zmiana `alpha.154`: systemowy koder Media Foundation potrafił przy źródle 22,05 lub 24 kHz wybrać 80 kb/s mimo żądania 128 kb/s. Tor stratny MP3/AAC normalizuje teraz takie wejście wysokiej jakości resamplerem do standardowych 44,1 lub 48 kHz przed kodowaniem i weryfikowany testem zachowuje wybraną przepływność. Resampling nie jest przedstawiany jako poprawa źródła. FLAC, WAV i tryb Oryginalny omijają tę normalizację; HLS w trybie Oryginalnym jest bez konwersji zapisywany w kontenerze `.ts`.
...
Zmiana `alpha.162`: ręczne nagranie można podzielić klawiszem `T` w odtwarzaczu Radia albo na wybranej stacji widoku Nagrywane. Aktualny koder jest finalizowany atomowo, po czym ten sam prywatny tor rozpoczyna nowy plik bez zmiany odsłuchu. Formularz `Shift+R` ma jawnie opisany sposób zapisu: jeden plik albo części, oraz aktywne tylko dla części pole długości w minutach. Całkowity czas planu nadal wyznacza jedno zakończenie; długość części wyłącznie dodaje wcześniejsze granice plików. Nie powstaje osobne `Shift+T`, ponieważ prowadziłoby do tego samego edytora.

Zmiana `alpha.299`: zakończone części nagrań tworzą trwały indeks **Nagrane pliki** w Bibliotece SQLite. `Alt+Shift+R` otwiera go z Radia albo Plików lokalnych, przełączając do sesji Pliki lokalne, ponieważ po finalizacji nagranie jest zwykłym plikiem. Lista łączy wszystkie skonfigurowane miejsca zapisu, ma najnowsze nagranie na początku i dziedziczy pełną semantykę lokalnych multimediów zamiast osobnych poleceń Radia. Indeks zapisuje stabilny identyfikator rekordu oraz czas zakończenia, więc zmiana nazwy w Bibliotece nie zrywa powiązania. Nie skanuje przy każdym otwarciu folderów ani nie pobiera plików z chmury. Migracja wstecz oznacza tylko pliki już obecne w Bibliotece i znajdujące się w dedykowanym folderze Radia albo folderze harmonogramu; nie klasyfikuje hurtowo współdzielonego folderu podcastów. `Alt+R` zachowuje odrębne znaczenie listy aktualnie trwających nagrań.

Zmiana `alpha.300`: `T` i `R` nie zmieniają pochodzenia aktywnego nagrania harmonogramowego. Podział `T` używa nadal rozwiązanego folderu, formatu, bitrate, schematu nazwy i granicy końca tego wystąpienia. Pierwsze `R` finalizuje część i oznacza wystąpienie jako tymczasowo zatrzymane; zegar nie uruchamia go ponownie samoczynnie. Drugie `R` tej samej stacji przed końcem okna wznawia ten harmonogram z jego ustawieniami. Szybkie drugie naciśnięcie jest zapamiętywane do czasu atomowego opublikowania pierwszego pliku. Brak wznowienia powoduje zwykłe przejście do kolejnego terminu po końcu okna. Polecenie globalnego zatrzymania oraz zamknięcie programu nadal świadomie kończą wystąpienie i nie pozostawiają stanu tymczasowego. Każda opublikowana część harmonogramu trafia do indeksu Nagrane pliki.

Zmiana `alpha.301`: pliki robocze transakcyjnego cięcia, których nazwa ma postać `.nazwa.amc-cut-identyfikator-before/after/result.rozszerzenie`, są wewnętrznym stanem operacji i nigdy nie mogą trafić do Biblioteki, historii ani bieżącego widoku. Dotyczy to także okresu, w którym obserwator folderu widzi plik przed zakończeniem FFmpeg. Sprzątanie jest ponawiane asynchronicznie przez kilka sekund, aby uwzględnić opóźnione zwalnianie uchwytów przez chmurę i program ochronny. Niesprzątnięty plik pozostaje niewidoczny dla użytkownika i jest wskazany w logu. Nie wolno automatycznie usuwać pozostałości przy następnym uruchomieniu, jeżeli właściwy plik źródłowy nie istnieje: część robocza może być wtedy jedynym materiałem do odzyskania. Kopia `.amc-backup` jest odrębna od plików roboczych. Od `alpha.409` domyślnie usuwa się wyłącznie kopię utworzoną w bieżącej udanej edycji, po sprawdzeniu zapisanego pliku. Opcja „Zachowuj kopie po edycji” pozostawia ją na dysku. Błąd lub niepewny wynik zachowuje kopię; starsze kopie nie są sprzątane.

Zmiana `alpha.302`: całkowita długość nagrania w edytorze harmonogramu ma dwa natywne pola liczbowe: **Godziny** od 0 do 168 i **Minuty** od 0 do 59. Każde pole ma własną, jednoznaczną nazwę dostępnościową, przyjmuje wpisaną liczbę oraz reaguje na strzałki góra/dół. Zero godzin jest prawidłowe, lecz suma nie może wynosić zero. Wewnętrzne `DurationMinutes` pozostaje formatem trwałym, dlatego dotychczasowe harmonogramy nie wymagają migracji i po otwarciu są bezstratnie rozkładane na godziny i minuty. Wiersz listy harmonogramów przedstawia czas jako minuty poniżej godziny, pełne godziny albo godziny z pozostałymi minutami.
...
poleceniem pobierania i nie zostaje przejęte przez Discogs. Planowane
`Ctrl+Shift+I` otwiera wyszukiwanie informacji katalogowych; `Ctrl+Alt+D` nie
jest używany, ponieważ uruchamia NVDA. Strzałka w prawo
...
Od `alpha.409` po sprawdzeniu rzeczywiście zapisanego pliku ta nowa kopia jest
domyślnie usuwana. „Zachowuj kopie po edycji” pozostawia kopię; błąd lub
niepewność również ją zachowuje. Starsze kopie pozostają nietknięte.
...
Biblioteki, Ulubionych albo innego widoku tej samej sesji zachowuje dźwięk.
Dzięki temu Radio na osobnym wyjściu może grać nadal, gdy użytkownik przegląda
inne dane. Wyłącznie jawny Escape, Shift+F6 lub przycisk powrotu stosuje
