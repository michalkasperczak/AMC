...
`Ctrl+F` otwiera wyszukiwanie w bieżącej usłudze, a `Ctrl+Shift+F` — wyszukiwanie globalne. Nazwa okna i nazwa dostępnościowa pola edycyjnego zawsze zawierają zakres, dlatego czytnik ekranu podaje tryb także wtedy, gdy nie odczyta poprawnie samej belki tytułowej. Enter w polu wykonuje zapytanie i przenosi fokus na pierwszy wynik; kolejny Enter otwiera wynik. Działania bezpośrednie pozostawiają wyniki otwarte. Po powrocie z wyniku globalnego program podaje bieżącą usługę i widok.

Po poprawnym zapytaniu natywna lista podaje etykietę wyniku i jego pozycję, np. „1 z 3”. Widoczna liczba wyników nie jest osobnym komunikatem na żywo. Przy włączonych szczegółowych podpowiedziach instrukcja klawiszowa jest pomocą wybranego elementu, dzięki czemu następuje po nazwie wyniku, a nie przed nią. Brak wyników pozostaje jawnym komunikatem.
