...
Harmonogram Radia będzie następnym modułem po testach `alpha.140`. Rekord przechowuje stabilny identyfikator stacji i bezpieczną kopię adresu, czas rozpoczęcia, strefę czasową, długość, tryb jednorazowy albo wybrane dni, folder, format MP3 oraz trzywartościową regułę wybudzania: dziedzicz, włącz, wyłącz. Lista oraz zapis są synchronizowane, plik lub tabela są aktualizowane atomowo, a ponowne uruchomienie normalizuje terminy cykliczne. Opóźniony start nagrywa tylko pozostałą część nadal trwającego okna; całkowicie pominięte wystąpienie nie nagrywa pełnej długości po czasie. Najbliższy termin wymagający wybudzenia utrzymuje jeden aktualny czasomierz Windows. Nagrywanie działa w AMC niezależnie od NVDA i używa istniejącego, odpornego toru strumieni oraz zapisu MP3.
