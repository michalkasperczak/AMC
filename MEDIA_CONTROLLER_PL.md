...
Nagrywanie radia jest świadomie uruchamianą funkcją lokalną do prywatnego użytku: w aktualnym interfejsie `R` zapisuje od chwili włączenia dekodowany strumień do wybranego formatu i folderu. Od `alpha.116` domyślnym wynikiem jest MP3 kodowany przez systemowy Windows Media Foundation, z jakością docelową 192 kb/s. Nie jest to kopiowanie oryginalnych segmentów transmisji: MP3, AAC, OGG i inne obsługiwane źródła przechodzą przez jeden dekodowany tor PCM, dzięki czemu zerwanie HLS lub zmiana kodeka nie pozostawia niespójnego kontenera. Zapis odbywa się do pliku roboczego `.amc-partial`, a docelowa nazwa powstaje atomowo dopiero po zamknięciu kodera. Zatrzymanie, zmiana stacji i kontrolowane zamknięcie aplikacji finalizują plik; awaria kodera usuwa niedokończone dane, jest oznajmiana bez identyfikatora technicznego i nie zatrzymuje radia. Funkcja nie uruchamia się automatycznie, nie dotyczy TIDAL, Spotify ani Apple Music, nie obchodzi DRM i pozostawia użytkownikowi odpowiedzialność za zgodność z prawem właściwym dla miejsca użycia.
...
Uzupełnienie `alpha.142`: aktywne okno AMC przechwytuje dokładnie `Ctrl+Shift+0` także w niskopoziomowym haku klawiatury, zanim kombinacja dotrze do WPF. Jest to celowe obejście dodatków czytnika ekranu pochłaniających skrót; nie jest globalnym presetem i nie działa, gdy AMC jest w tle. `Ctrl+0` zachowuje wyłącznie listę sesji. Nagrywanie ręczne bieżącej, słyszanej stacji działa przełącznikiem `R` tylko w odtwarzaczu Radia. `Shift+R` na stacji otwiera bezpośredni dialog nagrania czasowego: tryb domyślny rozpoczyna niezależne nagranie natychmiast na podaną liczbę minut, a po jego wyłączeniu dostępny jest przyszły termin i reguła powtarzania. Oba polecenia oraz menedżer planów muszą być widoczne w adekwatnych menu kontekstowych. Ręczna zmiana stacji w trakcie nagrywania wymaga jawnej decyzji o zakończeniu zapisu.
