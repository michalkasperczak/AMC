...
Zakładka jest lokalnym rekordem AMC wskazującym stabilny identyfikator sesji, element i pozycję, a w przyszłości także opcjonalną nazwę. Może więc wskazywać plik lokalny albo pozycję w materiale streamingowym, jeśli adapter potrafi ponownie otworzyć ten sam element i przewinąć go. Zakładki są z natury globalnym indeksem aplikacji, ale nie kopiują ani nie przejmują treści usługi. Od `alpha.68` w odtwarzaczu `B` tworzy szybką zakładkę, `Shift+Page Up/Down` przechodzi po zakładkach bieżącego materiału, a `Ctrl+B` otwiera globalną listę. Enter na jej pozycji przełącza sesję, otwiera element i ustawia czas. `Delete` usuwa rekord zakładki; `Shift+Delete` nie może z tego widoku usunąć źródłowego pliku. Zwykłe `B` na innych listach pozostaje nawigacją literową. `Ctrl+Shift+B` jest zarezerwowane dla późniejszej zakładki nazwanej.
...
5. Format i edycja nazwanych zakładek; podstawowy globalny widok i eksport pełnej kopii działają od `alpha.68`.
