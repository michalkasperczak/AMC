...
**Presety urządzeń nie są playlistami Radia internetowego.** W sesji WiiM lub innego adaptera, który deklaruje prawdziwe presety, planowane `Ctrl+P` otworzy listę presetów, a `Ctrl+Shift+P` przypisze lub zmieni preset zgodnie z możliwościami API. Projekt przewiduje dwanaście bezpośrednich miejsc odpowiadających klawiszom `1–0`, `-` i `=`, roboczo z modyfikatorem `Ctrl+Shift`; przed wdrożeniem wymagany jest test różnych układów klawiatury i możliwość konfiguracji. Przypisanie stacji internetowej do presetu WiiM należy do sesji i urządzenia WiiM, nie tworzy presetu w lokalnej sesji Radia.
