...
Pozostałe zatwierdzone wcześniej polecenia warstwy zachowują litery `R` — Radio, `M` — Miksy, `H` — Historia, `N` — Teraz odtwarzane i `O` — Wyjścia. Trzy wcześniejsze polecenia informacji zostały połączone w jedno **Właściwości i informacje** pod `Alt+Enter`. Nie ma ono domyślnego przypisania po prefiksie. Otwiera dostępne, tylko do odczytu okno z sekcjami Podstawowe informacje, Odtwarzanie, Techniczne i Źródło. Ctrl+A i Ctrl+C działają standardowo, Escape albo Enter zamyka okno i przywraca fokus. Każdy konflikt rozstrzyga edytor mapy, a polecenie może pozostać bez skrótu i być dostępne z menu oraz palety.
